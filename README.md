...
[![Rust](https://img.shields.io/badge/rust-stable%20%2F%20edition%202021-orange)](https://www.rust-lang.org/)
[![WASM](https://img.shields.io/badge/target-wasm32--unknown--unknown-blueviolet)](https://rustwasm.github.io/wasm-pack/)
...
力学・流体・熱・電磁気・量子・統計・天体の 7 ドメインを、それぞれ独立した `Solver` として Rust で実装し、`World` の下に統合したシミュレータ。物理コアは WebAssembly にコンパイルされ、Three.js 製のブラウザ統合エディタから編集・再生できる。

スケールごとに最良の理論を選び、理論間の受け渡しを `Coupling` として明示的に設計する——「スケールの階梯と有効理論」([docs/00-foundation/02-scale-ladder.md](docs/00-foundation/02-scale-ladder.md))が本エンジンの根本思想である。

## 特徴

- **7 ドメイン統合** — `sim_world::World` が有効化されたドメインを固定順(力学 → 熱 → 電磁 → 天体 → 回路 → SPH → 格子流体)で進め、Lie–Trotter 分割で結合処理を挟む。ドメイン間結合は 15 モジュール実装済み([crates/sim-coupling/src/](crates/sim-coupling/src/))。
- **検証可能性** — `#[test]` 644 件が、解析解テスト表([docs/21-verification/01-analytic-tests.md](docs/21-verification/01-analytic-tests.md))と保存則([docs/21-verification/02-conservation-laws.md](docs/21-verification/02-conservation-laws.md))に紐づく。`EnergyLedger` が実行中の数値誤差を常時可視化する。
- **決定論** — 固定タイムステップ・シード付き PRNG・状態ハッシュにより、同条件ならビット単位で同じ結果になる。リプレイ・共有・回帰テストの基盤。
- **近似の自己申告** — 各 `Solver` は使用中の近似を `Approximation { name, reason, doc, can_disable }` として申告し、エディタがバッジで表示する。
- **依存が実質ゼロ** — 線形代数・FFT・PRNG・PNG 書き出しまで自前実装。外部クレートは `serde`/`serde_json`、WASM 境界の `wasm-bindgen`/`js-sys`、ベンチの `criterion` のみ。
- **ブラウザ統合エディタ** — Edit/Play モード、Gizmo、Hierarchy / Inspector / Timeline / Console / Probe グラフ(CSV エクスポート)を備える([docs/23-frontend/01-editor.md](docs/23-frontend/01-editor.md))。
- **物理正確なパストレーシング** — 分光レンダリング、GGX マイクロファセット、binned-SAH BVH、MIS、コースティクス、CIE 等色関数 + ACES トーンマップ([crates/sim-render](crates/sim-render/))。
...
# 物理コアのテスト(全ドメインの解析解テスト・決定論テスト)
cargo test --workspace

# WASM をビルドして demo/pkg に出力する(demo/pkg は .gitignore 済み)
...
- **時間** — 再生・一時停止・指定 step 数のステップ実行、×1/8〜×128 の時間倍率。
...
| [sim-fluid](crates/sim-fluid/) | MAC 格子 Eulerian 2D/3D(PCG 圧力解法・マルチグリッド前処理・渦度強化)、WCSPH、浮力、空力・水力 |
...
| [22-roadmap](docs/22-roadmap/) | 実装フェーズ計画と機能チェックリスト |
...
回帰ゲートは merge-base を `git worktree` で取り出し、同一マシン上で作業ツリーと A/B 比較する。既定閾値 25% は実測のノイズ床(同一コードで約 6%)に基づく。
...
- **3D 格子流体の性能** — 64³ で 206.8 ms/step(設計予算 4 ms に対し約 50 倍)。マルチグリッド前処理 PCG([crates/sim-fluid/src/pressure_multigrid.rs](crates/sim-fluid/src/pressure_multigrid.rs))導入で約 3.5 倍縮んだが、SIMD・並列化・GPU が未着手なため予算には届かない。
