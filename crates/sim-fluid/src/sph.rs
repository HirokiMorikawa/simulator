...
//!   実験的に確認したため対称形を採用する。
//!
//! 曲面・角の境界は本近似の対象外。**この注記は当初「動的剛体結合は未実装のため
//! 影響なし」と書いていたが、`sim-coupling::SphRigid`(球剛体を境界粒子群で覆う)が
//! 実装済みなので影響はある**——球剛体の境界粒子は曲面をなすため、この平面近似の
//! 外側にいる(`SphRigid`のモジュールdocが記録している「動的シナリオでの定量検証の
//! 難しさ」の一因)。
