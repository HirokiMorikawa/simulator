...
//!
//! **群4で `joint::WheelJoint`(サス+駆動+操舵)を実装した**ので、
//! 「フルの `WheelJoint` は未実装」という前提は解消している。本モジュールは
//! **タイヤ力そのものの式**(路面とタイヤの間のスリップ→力)を担い、
//! `WheelJoint` は**車体と車輪を繋ぐ拘束**を担う——役割が違うので両方要る。
//!
//! 本モジュールは設計§9のPacejka簡易Magic Formula
