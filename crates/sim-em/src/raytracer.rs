...
//! 分岐トレース(深さ・パワー打切り)。光線束追跡(rayon並列化)・結像のスクリーン
//! ビニングは未実装(単一光線のエネルギー収支検証が目的のP4残りのスコープでは不要)。
//!
//! **CIE等色関数によるRGB変換は`optics`モジュールに実装済み**(`cie_x_bar`等)で、
//! 分光レンダリングは`sim-render`の`spectrum`モジュールが担う。このモジュール自身が
//! 波長サンプリングを持たない点は変わらない。
