...
//! 読めなくしても救いにならないためである。
//!
//! # このモジュールの適用範囲(**11ドメインへ配線済み**)
//!
//! 11ドメインの`raw_state`が持つ**可変長の数値配列**は、すべてこの符号化を
//! 通っている(`crate::scenario`モジュールdoc §「数値配列の符号化」)。
//! 符号化は`export.rs`の`encode_*_field`、復号は`scenario.rs`の
//! `decode_*_field`が入口で、後者は失敗を
//! `SceneError::InvalidValue`へフィールド名つきで写す。
//!
//! **固定長の配列は対象外**である。`[f64; 3]`の`gravity`・`external_force`・
//! `box_size`などは従来どおりJSONの数値配列で、要素数が3つ程度では
//! 生バイト化の利点が無く人が読めるほうが勝つ。
//!
//! 往復するコード例は本モジュールのテスト
//! `usage_example_grid_fluid_raw_state`(`#[cfg(test)]`、説明用の素描であって
//! 本番のスキーマそのものではない)を参照。
//!
//! ## `float_roundtrip`について(上の §「なぜ生バイトなのか」1. の訂正)
//!
//! 「`float_roundtrip`は全ドメインの移行が終わった時点で外せる」と書いたが、
//! これは**11ドメインの`raw_state`についてだけ正しい**。シーンJSONの他の大半
//! ——`bodies[].position`・`materials`・`thermal`・`circuit`・`astro`——は
//! 今も10進のf64であり、D24車のsave→load→60stepがハッシュ一致すること
//! (`tests/editor_acceptance.rs`)はそのパーサ挙動に依存している。
//! よって`float_roundtrip`は**外せない**。外せるようにするには剛体・材料側も
//! 同じ移行をする必要があり、それは別の話である。
//!
//! # 対応する要素型
//!
