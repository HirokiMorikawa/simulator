...
//! モジュールdoc参照)と、具体的な実装4種(`DissipationToHeat`・`JouleHeat`・
//! `BrownianForce`・`LorentzForce`、各モジュールdoc参照)を実装する。残る8種
//! (`BuoyancyDrag`・`GridFluidRigid`等、設計§3)・sub-iteration剛性閾値表
//! (設計§2規則3)は後続増分で追加する。
