...
//! モジュールdoc参照)と、具体的な実装5種(`DissipationToHeat`・`JouleHeat`・
//! `BrownianForce`・`LorentzForce`・`InductionCoupling`、各モジュールdoc参照)を
//! 実装する。残る7種(`BuoyancyDrag`・`GridFluidRigid`等、設計§3)・sub-iteration
//! 剛性閾値表(設計§2規則3)は後続増分で追加する。
