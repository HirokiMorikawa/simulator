...
//!
//! 双方向性は`apply`ひとつで実現する: (1)まず`boundary_force`(自分の境界粒子群の合計)を
//! 剛体への反作用力として適用し、(2)続けて境界粒子群の位置を剛体位置に更新する。
//!
//! **群5でこのdocの記述を訂正した**(実装は変えていない)。移行前のこのdocは上記を
//! 「1step遅れの縮約」と書いていたが、`World`経由で走らせる限り**反作用力に1step遅れは
//! 無い**。`World::step`は各ドメインソルバを進めた**後**に post 相(既定では`apply`)を
//! 呼び、`SphFluid`はそのドメインソルバの一員なので、ここで読む`boundary_force`は
//! **今stepのSPHステップが確定させた値**である。「前stepの値を読む」という記述は、
//! `World`が結合レジストリを持つ前に呼び出し側が手で`apply(); sph.step();`と書いていた
//! 頃の名残だった。
//!
//! 残る遅れは**境界粒子位置のほう**で、これは`World`のドメイン固定順序(mechanics →
//! … → sph)に由来する: post 相で書いた位置は次stepのSPHステップが読むが、その時点で
//! 剛体は既に力学ステップ1回ぶん($v\,\Delta t$)先へ動いている。`apply_pre`へ移しても
//! 同じで(pre 相と sph の**間に**力学ステップが挟まるため)、pre/post の2相しか無い
//! 現行APIでは解消できない — 群5で実際に pre 相へ移して測り、遅れ量が変わらないことを
//! 確認した上でこの記述に落ち着いた。
//!
...
impl Coupling for SphRigid {
    ...
    fn apply(&mut self, world: &mut DomainStates, dt: f64) {
        ...
        // 反作用: 今stepのSPHステップで確定した境界粒子群への合力を剛体へ適用
        // (`World`は全ドメインソルバの後に post 相を呼ぶ、モジュールdoc参照)。
