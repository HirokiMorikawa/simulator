...
//! 引き続き未実装(`quad.rs`モジュールdoc参照)。分割戦略は最長軸の重心中央値
...
//!
//! `Scene::closest_hit`への配線は本増分で完了した(以前は「多数物体シーンが
...
impl Bvh {
    /// `primitives`が空なら常にヒットしない`Bvh`を返す。
    pub fn build(primitives: &[Primitive]) -> Bvh {
        if primitives.is_empty() {
            return Bvh { root: None };
        }
        let leaves: Vec<(usize, Aabb, Vec3)> = primitives
            .iter()
            .enumerate()
            .map(|(i, p)| {
