...
//! 同じ判断)。分光は`trace_spectral`(波長を経路へ伝える)と`spectrum`モジュール
//! (hero wavelength法・CIE等色関数)で実装済み——`trace`自体は引き続きモノクロの
//! 放射輝度スカラーを返す。
//!
...
//! (エリアライト)を扱うには多重重点サンプリング(MIS)が必要になる——**群6で
//! `Scene::enable_mis`として実装した**。既定は無効なので、この`PointLight`の
//! 単純加算経路は移行前のまま成立し続ける)。
...
///
/// **群6でMISを実装した**(`Scene::enable_mis`)。`enable_mis`を呼んだシーンでは
/// 面光源へもNEEを行い、べき乗ヒューリスティック($\beta=2$)の重みで二重計上を
/// 打ち消す。小さく明るい光源では標準誤差が半分以下になることを実測した。
///
/// **既定は移行前のまま**(MIS無効): 面光源については**NEEを一切行わない純粋な
/// BSDFサンプリングのパストレース**で、レイがこの面に当たったときに`radiance`を
/// 加算するだけなので二重計上が原理的に起こらない(不偏。分散はMIS併用より
/// 大きいが、R4の検証は高サンプル数の点推定なので許容できる)。この既定のおかげで
/// R1–R7・R4の既存テストは`enable_mis`の導入後もビット単位で不変である。
...
mod tests {
    ...
    /// 崩れる既知の限界。群6で`trace_with_roulette`(不偏な確率的打切り)を追加したが、
    /// **これは打切りバイアスを分散へ移すもので、全反射で閉じ込められた経路の
    /// エネルギーを回収するわけではない**——このテストが臨界角を避ける理由は変わらない)。
    #[test]
