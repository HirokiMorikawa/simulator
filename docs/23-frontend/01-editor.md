...
リサイズができる(タブ化・切り離しは未実装、§1.7)。レイアウトは既定を含めて 4 種の
プリセットを持つ。
...
パネルに属さない、エディタ全体に効く約束事。設計当初は明示していなかったが、
実装と QA([reviews/2026-08-04-editor-qa.md](../reviews/2026-08-04-editor-qa.md))を
経て「ここが崩れると他が全部使いにくくなる」と分かった層である。

- **パネルのリサイズ**: Hierarchy / Inspector の幅と Console 行の高さを、パネル境界の
  スプリッターで変える。マウスのドラッグに加えて**矢印キーでも動かせ**、ダブルクリックまたは
  `Home` で既定へ戻る。値は localStorage に残り、次回の起動でも保たれる。
  レイアウトプリセットが持つ変数(Console 行の高さ)はプリセット切替で上書きが外れる。
  タブ化・切り離しは引き続き未実装。
- **キーボードだけで到達できること**: すべての操作対象に `:focus-visible` の輪郭を出す。
  Hierarchy は上下キーで辿れ(選択がフォーカスに追従する)、Console / Project のタブは
  左右キーで移動できる。ツリーとタブは `role="tree"` / `role="tablist"` を名乗り、
  `aria-selected` が見た目の状態と一致する。
- **ショートカットの可視化**: `?` または `F1` で一覧を開く(ツールバーの `?` からも同じ)。
  一覧の定義はショートカットを実装している `keydown` ハンドラと同じファイルに置く
  ——「`title` には書いてあるが実装が無い」(不具合 7)の再発を見つけやすくするため。
- **失敗の伝え方**: 即時通知はトースト(非ブロッキング、8 秒で消える)で出し、同じ文面を
  Console の Errors タブへ恒久的に残す。`window.alert` は使わない
  ——操作を止め、閉じた瞬間に文面が消えるため「即時に気づける」と「後から見返せる」の
  どちらも中途半端になる。
- **「空」と「壊れている」を分ける**: 観測データが無い Probe Graphs、ボディを持たない
  シーンの Inspector、絞り込みで 0 件になったシーンギャラリーは、空白ではなく
  「何が無いのか / 次に何をすればよいか」を出す。押しても何も起きないボタンは
  `disabled` にする。
- **起動中であることを示す**: wasm の取得とコンパイルのあいだはオーバーレイを出し、
  失敗したらその場に理由を出す。
