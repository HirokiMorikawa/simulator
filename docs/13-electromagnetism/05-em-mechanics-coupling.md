...
帯電剛体・荷電粒子デモに直接適用。**荷電粒子・帯電点質量(接触なし)の時間積分は
Boris pusher を用いる**([01-math/03-integrators.md](../01-math/03-integrators.md) §2.6):
磁場回転を厳密にノルム保存するため「磁場は仕事をしない」が離散レベルで成立し、
サイクロトロン検証(E2、§7)の速さ一定 abs 1e-9 を構造的に満たす。
接触インパルスを持つ帯電剛体はローレンツ力を force generator として剛体積分器側で扱う。
...
- サイクロトロン: 一様 B 中の荷電粒子の半径・周期(解析値 ± 0.5%、
  エネルギー保存: 磁場は仕事をしない → 速さ一定 < 10⁻⁹。担当積分器は Boris pusher —
  [01-math/03](../01-math/03-integrators.md) §2.6、[21-verification/01](../21-verification/01-analytic-tests.md) E2)。
...
- ホットスポット: モーター結合の 2 時間スケール進行、ローレンツ力評価。
- 目標アルゴリズムとオーダー: 回路 sub-step + 力学 step。$O(N_{coupling})$。
- SoA レイアウト: 結合対(回路素子↔ヒンジ)を配列化。
- 並列化単位: 独立結合を並列(通常少数)。
- SIMD 対象カーネル: ローレンツ力の荷電体バッチ。
- GPU 適性: 低。
- ベンチ: モーター・発電・渦電流(E6/E7)。
