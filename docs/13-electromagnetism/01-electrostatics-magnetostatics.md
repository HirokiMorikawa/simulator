...
- **荷電粒子・帯電点質量の運動積分は Boris pusher を用いる**
  ([01-math/03-integrators.md](../01-math/03-integrators.md) §2.6)。磁場回転を
  厳密にノルム保存し、サイクロトロン(E2)の「速さ一定」を構造的に満たす。
  剛体接触を持つ帯電体は従来どおり force generator + semi-implicit Euler。
...
- 軌道: 電場中の荷電粒子の放物軌道、一様磁場中のサイクロトロン半径 $r = mv/(qB) \pm 0.5\%$
  ...
  運動積分は Boris pusher — [01-math/03](../01-math/03-integrators.md) §2.6)。
...
- ホットスポット: 源の直接和(クーロン・双極子)。
- 目標アルゴリズムとオーダー: 直接和 $O(N^2)$(源は数十で十分)。多数化時は Barnes-Hut(天体と共有の発想)。
- SoA レイアウト: 電荷・双極子モーメントを配列化。
- 並列化単位: 源ペアを rayon 分割 + 順序固定リダクション。
- SIMD 対象カーネル: 逆二乗・逆四乗の距離計算バッチ。
- GPU 適性: 中(多数源で有効)。
- ベンチ: 帯電多体・磁石アレイ。
