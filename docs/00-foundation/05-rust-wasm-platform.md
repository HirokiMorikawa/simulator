...
- **決定論の制約**: 並列リダクションは**同一スレッド数で決定的**
  (スレッド数から決定的に構成したチャンク分割・結合順)。スレッド数を変えた実行や並列=逐次の
  ビット一致は要求せず、許容誤差で照合する([20-integration/02](../20-integration/02-determinism-replay.md) §5 の 2 階層)。
...
- 上表の値は**シングルスレッド wasm を基準**とする(§6.1 の配布既定と一致)。並列有効時の
  余裕は予算に織り込まない(並列はオプション構成のため)。

### 5.1 同時アクティブ構成規則 — シーンクラス別予算

上表のワークロード別予算は**単独アクティブ時**の値であり、単純加算すると物理予算 10 ms を
超える。同時アクティブの扱いをシーンクラスで規則化する:

| クラス | 構成 | 保証 |
|---|---|---|
| **S**(単一ドメイン) | 上表のワークロードが単独アクティブ | 60 fps 保証(上表の予算をそのまま適用) |
| **M**(2–3 ドメイン結合、例 D18 氷と飲み物) | **物理合計 10 ms 以内に収まる構成のみ** | 60 fps 保証(構成例: 剛体 3 + 格子流体 4 + 熱 1 + 結合 1 ≈ 9 ms) |
| **L**(統合シナリオ、例 D37 再突入・Phase C 統合テスト) | 合計が 10 ms に収まらない構成 | **リアルタイム保証の対象外** |

- クラス L の 2 択と既定: 「規模を落とす(格子解像度・粒子数のプリセット)」か
  「**スロー再生**(物理時間 < 実時間、固定 dt のまま 1 描画フレームあたりの物理ステップ数を
  減らす)」— **既定はスロー再生**。精度は落とさず時間で払う方針であり、
  [01-vision.md](01-vision.md) §5.4「静かに精度を落とすことを禁じる」と整合する
  (スロー再生倍率は UI に常時表示 — 静かに遅くもしない)。
- シーンのクラスはシーン定義時に宣言し、UI に表示する。クラス M 宣言のシーンが実測で
  10 ms を超えたら自動でクラス L 扱い(スロー再生バッジ)とし、ベンチ回帰として記録する。
- 「どの構成なら 60 fps を約束するか」はクラス定義から機械的に決まるため、シーンごとの
  場当たり調整は発生しない([06-performance-strategy.md](06-performance-strategy.md) §5)。
...
- CI: `cargo fmt --check` / `cargo clippy -D warnings` / `cargo test`(全ドメインの解析解テスト)/
  wasm ビルド / demo ビルド。決定論テスト(同一シード 2 回実行のハッシュ一致 — 階層 1)を必須ゲートにする。
  wasm ⇔ ネイティブの照合は**許容誤差ベース**(階層 2、[20-integration/02](../20-integration/02-determinism-replay.md) §5)。
- 超越関数は **libm 準拠、コンパイラは既定設定**(FMA 有効)でビルドする
  ([20-integration/02](../20-integration/02-determinism-replay.md) §5 の階層 1 は
  同一バイナリ内なので libm 実装差・FMA の有無は影響しない)。
- コアで禁止するもの(clippy/lint + レビューで強制): `std::time`・`rand::thread_rng`・`HashMap` の反復順序依存
  (決定論のため。反復するマップは `BTreeMap` か挿入順 `Vec`)。

### 6.1 配布戦略 — シングルスレッド既定・COOP/COEP 条件付き並列

- **既定はシングルスレッド wasm**: SharedArrayBuffer を使わないため COOP/COEP ヘッダが不要で、
  GitHub Pages 等の**静的ホスティングにそのまま配布できる**。性能予算(§5)はこの
  シングルスレッド構成を基準に定義する。
- **並列 wasm はオプション**: `wasm-bindgen-rayon` による並列化は、COOP/COEP
  (`crossOriginIsolated`)を設定できるホスティングでのみ有効化する。ビルドは
  シングルスレッド版・並列版の 2 種を生成し、起動時に `crossOriginIsolated` を検査して
  ロードを切り替える(検査結果は設定として記録 — リプレイの「同一設定」に含める)。
- 並列有効時の決定論は階層 1(同一バイナリ・同一設定・同一スレッド数)で保証
  ([20-integration/02](../20-integration/02-determinism-replay.md) §5)。
- ネイティブ(`cargo test`・ベンチ・リファレンス実行)は常に並列可。
