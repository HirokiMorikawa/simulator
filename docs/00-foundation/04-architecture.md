...
位置量の正準表現は「**所属フレーム ID + そのフレームのローカル座標 f64**」である
([02-scale-ladder.md](02-scale-ladder.md) §2.2)。単一フレームのシーンでは
ルートフレームのローカル座標 = 従来のワールド座標であり、意味論は変わらない。
グローバル座標は正準状態ではなく、必要時にフレーム変換で導出する派生量である。

この不変条件により、スナップショット・リプレイ・並列化・undo が well-defined になる
(状態のコピーが複数あれば「どれが正か」の曖昧さで壊れる)。

**(2) エネルギー台帳 — 残差の定義とトレンド監視**

$$R_{\text{numerical}}(t) \;\equiv\; E_{\text{total}}(0) \;+\; W_{\text{injected}}(t) \;-\; \sum_{d \in \text{Domain}} E_d(t)$$

- $E_d$: 各ドメインの全形態エネルギー(運動・ポテンシャル・弾性・熱・電磁場・化学…)
- $W_{\text{injected}}$: モーター・ユーザー操作等の外部注入(符号つき)
- $R_{\text{numerical}}$: **ステップ前後のエネルギー差分から既知の仕事・熱を引いた残差**。
  独立に計算する量ではなく、上式で**定義される**量である(恒等式は残差の定義式であって、
  それ自体はテストにならない)。Baumgarte 偽仕事・
  移流散逸など**既知**の数値散逸は内訳として項別に記帳し、残差からは分離する。

台帳の役割は「不明なエネルギー漏れの常時**検出器**」ではなく、残差の**トレンド監視指標**である:

- 残差の時系列を記録し、(i) 単調増大・階段状ジャンプ等の**パターン検知**、(ii) シーンあたり
  **累積残差の緩い上限**、の 2 つだけをテスト対象にする。項別閾値の手動調整には依存しない。
- **バグ検出**の役割は、解析解が既知のシナリオに対するドメイン別**保存則テスト**
  ([21-verification/02-conservation-laws.md](../21-verification/02-conservation-laws.md))が担う。
  両者の役割分担の契約は §1.4.4。

World の第一級の役割は、この帳簿(`EnergyLedger`)を全ドメイン・全結合を横断して
一貫して記帳できるように提供・強制することである。
...
### 1.4.4 保存量の橋の対応則(Coupling との契約)と検証の役割分担

Orchestrator は Coupling ごとに「出所ドメイン → 行き先ドメイン、量 $Q$」のペアを台帳へ**2 度記帳**する
($Q$ を出所から引き、行き先に足す)。この対応則が破れる Coupling は Coupling 実装のバグとして
デバッグビルドで診断される — これは §1.1.2 (2) のエネルギー台帳の Orchestrator 側からの担保。

エネルギー検証の役割分担:

| 仕組み | 役割 | テスト対象 |
|---|---|---|
| ドメイン別保存則テスト([21-verification/02](../21-verification/02-conservation-laws.md)) | **バグ検出**(解析解が既知のシナリオで保存量を検算) | 保存量の絶対誤差・ドリフト閾値 |
| 対記帳診断(本節) | **結合バグ検出**(取り出しと注入の不一致をデバッグビルドで即検知) | 対記帳の差分ゼロ |
| エネルギー台帳残差(§1.1.2 (2)) | **トレンド監視**(不明残差の推移を観測) | パターン検知 + 累積残差の緩い上限のみ |

台帳残差を項別閾値でバグ検出に使うことはしない(恒真式化を避けるため)。
...
- **位置は所属フレームのローカル座標**: 剛体・粒子の position 配列は
  所属フレームのローカル座標 f64 を持ち、`frame: Vec<FrameId>` を状態に含める
  ([10-mechanics/01](../10-mechanics/01-rigid-body.md) §3、[02-scale-ladder.md](02-scale-ladder.md) §2.2)。
  ソルバ内の演算(衝突・接触・積分)は同一フレーム内で閉じ、フレーム跨ぎはフレーム階層機構
  (詳細設計は後続の横断設計文書)だけが扱う。
...
- **フレーム階層はコアで予約された機構**であり、天体スケール(L5)はその具体化である
  (「新ドメイン追加が既存ソルバ変更を要求しない」原則の例外ではない — 位置表現が最初から
  フレーム ID + ローカル座標であるため、天体対応は新フレームの追加として実現される)。
...
- **頻度**: 既定 1 s 間隔(120 step ごと)の**リングバッファ N 面**(既定 N = 8、直近 8 s 分)。
- **サイズ見積り式**: 1 面のサイズ $S \approx \sum_d (\text{状態要素数}_d \times 8\,\mathrm{B})$
  (f64 のビット直列化、§1.1.3 の $\Sigma_t$ 全体)。代表構成の具体値:
  64³ 格子流体(MAC 速度 3 成分 + 圧力 + スモーク ≈ $64^3 \times 5 \times 8$ B ≈ 10.5 MB)+
  SPH $2\times10^4$ 粒子(位置・速度・密度 7 f64 ≈ 1.1 MB)+ 剛体 500 体(~30 f64 ≈ 0.12 MB)
  → **1 面 ≈ 12 MB、8 面 ≈ 96 MB**。
- **メモリ予算**: スナップショット合計の**上限 256 MB**。見積りが上限を超える構成では
  保存間隔を自動延伸(1 s → 2 s → 4 s、面数は維持)し、延伸を UI に表示する
  (黙って巻き戻し可能範囲を減らさない)。
- **巻き戻しコスト**: 復元は 1 面のデシリアライズ(≈ 12 MB の memcpy 級、~16 ms)であり、
  対話操作として許容範囲。スナップショット取得も同オーダーのため 1 s 間隔なら
  フレーム予算の 1% 未満に償却される(取得はステップ境界で同期実行 — 決定論維持)。
