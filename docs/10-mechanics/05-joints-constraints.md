...
- ホットスポット: prepare(ヤコビアン・有効質量・コレスキー)と solve_velocity の反復。
- 目標アルゴリズムとオーダー: PGS + ブロックソルバ。長いチェーンは反復増。将来は小島の直接法(LDLᵀ)。
- SoA レイアウト: 拘束を種別ごとに配列化(構造体は行数が異なるため種別分離)。
- 並列化単位: 接触と同じ島並列。ラグドール=島単位。
- SIMD 対象カーネル: 3×3 ブロックソルバ、ヤコビアン積。
- GPU 適性: 低〜中(チェーンの逐次依存が強い)。
- ベンチ: 二重振り子・10 リンクチェーン・ラグドールで計測。
