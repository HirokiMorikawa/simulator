...
- ホットスポット: sequential impulses の反復(velocity/position iterations)。
- 目標アルゴリズムとオーダー: PGS $O(\text{反復} \times \text{接触数})$。warm starting で反復数削減。
- SoA レイアウト: 接触制約を配列化(法線・接線質量・累積インパルス)。
- 並列化単位: **接触島**単位で並列(独立島は並列、島内は逐次で決定論保持、[06] §2.2)。
- SIMD 対象カーネル: 有効質量計算、インパルス適用のバッチ。
- GPU 適性: 中(グラフ彩色による並列 PGS は可能だが決定論の確保が要注意)。
- ベンチ: 4〜8 段スタック・ニュートンのゆりかごで反復収束と時間を計測。
