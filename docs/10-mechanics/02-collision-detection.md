...
**退化ケースの扱い**。分岐条件・退化時の扱い・
フォールバックを固定する(すべて決定的判定):

| 分岐条件 | 退化の内容 | 扱い / フォールバック |
|---|---|---|
| $\lvert\mathbf{e}_i\times\mathbf{e}_j\rvert^2 < 10^{-10}$ | 辺×辺軸の平行退化(軸方向が不定) | その軸を候補から除外し面軸のみで判定(既述)。全 15 軸が退化する完全整列は面軸が必ず残るので起きない |
| クリップ結果が 0 点 | 数値誤差で入射面の全頂点が参照面外 | **最深サポート点 1 点**のマニフォールドにフォールバック(点接触として次ステップで再クリップ) |
| クリップ結果が 5 点以上 | 面同士の大面積接触 | 最深点 1 + 残りから面積最大化の 3 点を選ぶ 4 点縮約。同点は feature_id 昇順でタイブレーク(決定論) |
| 接触点間距離 < $10^{-3}\,r_{shape}$ | 頂点/辺接触で点が縮退 | 重複点をマージし 1〜2 点で継続(warm start は feature_id 一致点のみ引き継ぐ) |
| 貫入 > 0.5 × 最小半厚 | 深い貫入(初期条件の重なり・押し込み) | 法線は最小重なり軸のまま、位置補正の 1 ステップ補正量をクランプ([03-contact-solver.md](03-contact-solver.md) §4.5)して爆発的射出を防ぐ。診断イベント記録 |
| static 同士・自己ペア | 無意味ペア | broadphase で除外(フィルタ規約) |
