...
**難所: マルチグリッド前処理と不規則境界**。圧力ソルバは **PCG が既定**
(前処理は incomplete Cholesky / Jacobi)。性能目標のマルチグリッド前処理(§10)は
不規則な solid 境界(剛体のボクセル化で毎ステップ形が変わる)で粗格子化が壊れやすい:

- **分岐条件**: 粗レベルのセル 8 子が solid/fluid 混在する割合が、そのレベルで 30% を超える
  (トポロジからの静的判定 — 実行時の残差測定に依存しない = 決定論)。
- **退化時の扱い**: 混在セルの粗格子係数は子セルの流体体積率で平均(係数平均法)。
  Dirichlet(自由表面)が子に 1 つでもあれば粗セルも Dirichlet(境界条件の支配性を保存)。
- **フォールバック**: 分岐条件を超えたレベル以下の粗格子化を打ち切り、そのレベルを
  最粗として V サイクルを短縮する。最悪時(全レベル打ち切り)は **Jacobi 前処理の PCG に
  退行**する — 収束は遅くなるが正しさと決定論は保たれる(反復数上限は固定、
  未収束は残差を診断イベントで報告)。
- 実装順序: Phase 3 は PCG(IC(0))のみで F7–F9 を Green にし、マルチグリッド前処理は
  性能ベンチ(64³ 4 ms 予算)が要求したときに導入する(機能でなく性能の最適化)。

**実装状況(群10)**: 3D はマルチグリッド前処理 PCG(MGPCG)を実装済み
([crates/sim-fluid/src/pressure_multigrid.rs](../../crates/sim-fluid/src/pressure_multigrid.rs))。
上の 4 点をそのまま実装してある——分岐条件 30%、係数平均法、Dirichlet の支配性、
打ち切りフォールバック。実装で確定させた点が 3 つある:

- **平滑化は減衰 Jacobi($\omega=6/7$)**。Gauss–Seidel の方が 1 掃引あたりは強いが、
  Jacobi は掃引順序に依存しないため PCG が要求する $M=M^\top$ を構造的に保証できる。
  制限は延長の転置 $R=\frac18P^\top$ として書き、対称性を実装で担保している
  (対称性・正定値性は乱数ベクトルで機械的に検証: `tests/pressure_multigrid.rs`)。
- **粗格子作用素は再離散化**(Galerkin 積 $RLP$ ではない)。不規則境界では Galerkin 積の
  疎構造が 7 点に収まらず、matrix-free の利点を失うため。
- **分岐条件の分母**は「そのレベルで未知数を持ちうるセル」(全子が Solid でない粗セル)。
  全域が固体の広い領域で混在率が薄まり、規則が効かなくなるのを避けるため。

2D(`grid_fluid.rs`)は前処理なし PCG のままである。2D は 64² で既に予算内に収まっており、
「性能ベンチが要求したときに導入する」という上の実装順序が、まだ要求を出していない。
...
| 解像度 | 群9(前処理なし PCG) | 群10(MGPCG) | うち圧力投影 | 投影の反復数 |
|---|---|---|---|---|
| 32³ | 68.9 ms | 24.9 ms | 11.4 ms | 7 |
| 64³ | 716.8 ms | **206.8 ms** | 91.1 ms | 7 |
| 128³ | (未測定) | 1840 ms | 935 ms | 8 |

上の「目標アルゴリズムとオーダー」は達成した——**反復数が解像度にほぼ依存しない**
(体積が 8 倍になっても 7〜8 反復)。群9は 32³→64³ で時間が 10.4 倍(超線形)だったのに対し、
群10は 8.3 倍・64³→128³ で 8.9 倍とほぼ線形である。

**予算 4 ms には 64³ でまだ約 50 倍届かない**(群9の約 200 倍からは縮んだ)。
残差は上のリストのうち未着手の項目——SIMD カーネル、rayon による格子タイル並列、
WebGPU——であり、いずれもアルゴリズムではなく実行基盤の話である。反復数が解像度に
依存しなくなった以上、ここから先は台数効果で素直に縮む。

移流側もこの群で整理した(352 → 110 ms @64³)。周期ラップ(整数除算)と `clamp` を
三線形補間の 8 点ごとに踏んでいたのを軸ごとに 1 度だけ解くようにしたもので、
離散化は 1 ビットも変えていない。上のリストの「キャッシュブロッキング/タイリング」は
まだ入れていない。
