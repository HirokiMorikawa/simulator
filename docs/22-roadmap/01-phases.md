...
本計画は **AI 主導の一気通貫開発**を前提とする(対策方針書
[reviews/pr1-review-countermeasures.md](../reviews/pr1-review-countermeasures.md) §0 決定 2・決定 4)。
...
- 工程は本計画の現行構成を維持し、複雑化させない。ウェーブ局所 TDD への再編・各ウェーブ末の
  デモリリース規約・実装設計スパイク・独立トラック化・多層 CI 制度は導入しない。
- この開発体制の決定により、レビュー指摘 A-1(規模見積り不在)・B-1(Phase A の BDUF)・
  B-3(ユーザー価値の到達点の遠さ)・D(過剰設計のパス占有)は**対応不要**と判断済み
  (同方針書 §2・§3 に理由つきで記録)。
...
- **CI ゲート**: 決定論(全シーン 2 回実行一致 + スナップショット再開一致 + 並列=逐次一致)、
...
各フェーズ/ウェーブの完了条件と運用ルール:
1. 対象テスト全緑 + 収束次数 ◆。
2. 決定論 CI(同一シード一致・スナップショット再開一致・並列=逐次一致、
   [20-integration/02-determinism-replay.md](../20-integration/02-determinism-replay.md))。
3. 保存則 CI(residual 閾値内)。
4. 性能予算監視([00-foundation/05](../00-foundation/05-rust-wasm-platform.md) §5)+ ベンチ回帰なし。
5. 実装が設計から乖離したら**設計書を先に改訂**する。改訂義務が発動するのは
   **アルゴリズム変更・不変条件変更・API 変更・合格基準変更**のいずれかに該当する乖離のみ。
   それ未満のコードレベルの差異(定数の微調整・関数分割・命名・データレイアウト等)では
   発動しない(粒度規約 — 対策方針書
   [reviews/pr1-review-countermeasures.md](../reviews/pr1-review-countermeasures.md) §3 A-2)。
6. **レビュー指摘への対応は 2 段階で行う**(同方針書 §3 R-1):
   指摘 → 対策方針書(全指摘について対応する/しないを判断し、対応不要の判断も理由つきで
   記録する。黙殺しない)→ 設計文書の改訂 PR。設計フェーズでは、同方針書 §5 の改訂 PR 群が
   完了して初めて実装開始ゲート
   ([00-foundation/01-vision.md](../00-foundation/01-vision.md) §4)を通過できる。
