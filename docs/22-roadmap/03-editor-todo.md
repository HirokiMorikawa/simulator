...
- [x] `World → Scenario` の逆写像を実装する(全ドメイン完了)
  ...
  **1 ULP ずれる**ことを実測。JSON文字列を経由する往復に必須)。
  ...
  分けて検証している。
  - **base64+LE生バイト符号化の土台増分**: `raw_state` の数値配列を
    JSONの数値配列から base64+リトルエンディアン生バイトへ移すための
    符号化ユーティリティ `crates/sim-world/src/raw_bytes.rs` を追加した
    (`encode_f64_le_base64`/`encode_bool_bitpacked_base64`/`encode_i8_base64` と
    それぞれの復号)。主たる動機は**サイズではなく厳密性**で、10進テキストを
    経由しない表現なら f64 の往復が JSON パーサの丸め挙動に依存しなくなる
    (現状は `float_roundtrip` フィーチャに依存している)。base64 は
    `sim_render::png` が deflate を自前で書いた前例に倣い自前実装、復号は
    RFC 4648 §3.5 が実装依存とする非正準な符号語まで拒否する厳格版。
    この増分では**どのドメインにも配線していない**(土台のみ)。
  - **スナップショット一本化増分(構築レシピの撤去)**: 上の土台を実際に配線し、
    併せて**構築レシピ形式を11ドメインすべてから撤去**して `raw_state` を
    必須(`Option` ではない)にした。
    - **なぜ二重表現を畳んだか**: レシピは*人が手でシーンを書くための便宜*
      として入っていたが、シーンJSONのレシピ構文を通る利用者は
      `from_scenario` とチェックイン済みギャラリーシーンだけだった
      (構築の本来の入口は `World` の Rust API `enable_*`)。
      そのシーンを全件 `raw_state` へ移した時点で利用者が1つも残らないため、
      **シーンファイルは純粋なスナップショット**という形に畳んだ。
      往復の忠実性という一番効く用途では、レシピは原理的に劣る表現である
      (分散した波束はもはやガウス型ではない)。
    - **払った代償(正直な記録・当初の想定を訂正)**: 着手時の想定は
      「11ドメインはどれもエディタUIに構築の口があるから手書きJSONの
      レシピは不要」だったが、**実際に `sim-wasm` を確認すると口があるのは
      `grid_fluid` と `sph` の2つだけ**だった(`enable_grid_fluid_2d_domain`・
      `spawn_fluid_block`)。残る9ドメインはエディタからも新規に置けない。
      したがってこの移行は「UIがあるから安全」ではなく、
      **新規シーンを手書きJSONから起こす経路を畳んだ**というのが正確な記述である
      (もともとUIの口が無かったので新たに穴が空いたわけではない)。
      新規シーンを起こすには Rust で `World` を組んで `to_scenario` する
      (`scenes/*.json` の移行自体がその手順)。これらのドメインに作成口が要るなら
      **エディタ側にUIを足す**のが筋で、シーンJSONへレシピを戻すことではない。
    - **残したフィールド**: `raw_state` の外に残るのは*その配列を復号するのに
      要る構造パラメータ*だけ——`grid_fluid`/`grid_fluid_3d`/`fdtd` の
      `nx`/`ny`/`nz`/`h` と、`fdtd.pml`(係数表8本を決定的に組み直す2値)。
      これらは「初期状態の作り方」ではなく**データの形**なのでレシピではない。
    - **`to_scenario` が `Result` になった**: 符号化に非有限値を拒否する版
      (`encode_*_finite`)を使うため。発散した `World` を「妥当なシーン」として
      静かに書き出せてしまうより、書き出しに失敗するほうがよい。
      wasm 境界へは `WasmError::ScenarioExport` を足して通した。
      `[f64;3]` 用の検査版が無かったので `encode_vec3_le_base64_finite` を追加
      (位置・速度は発散が最初に現れる場所で、ここを素通しにすると検査の意味が薄い)。
    - **シーン移行と検証**: 移行対象は15ファイル。**11ドメインのセクションだけ**を
      差し替えている(シーン全体の再エクスポートでは `bodies[].name` が `body_0`
      のような機械名に潰れ materials・probes の参照も巻き込まれるため)。
      `scenes/index.json` の**全43シーン**について移行前後で `state_hash` が
      $t=0$ と60step後の両方で完全一致することを確認した。
    - **正直な代償(ファイルサイズ)**: スナップショットは実データなので、
      移行した15ファイルの合計は **7.1 KB → 1.81 MB**(+約 1.76 MB)になった。
      内訳の大きいものは D27(二重スリット、256×128 の波動関数3配列)1.05 MB・
      D14c(3D煙、24×12×12 の速度3成分+煙密度)259 KB・
      D29(FDTD 81×81 の Ez/Hx/Hy + PML分離場)209 KB・
      D14b(格子流体 64×32 に円柱固体)110 KB。
      固体を1セルも置かない格子は `solid_cells`/`solid_velocity` を空文字列にして
      中身のないゼロの base64 を書かない緩和を入れてある(復元結果はビット単位で
      同一、D15対流で 32 KB 相当を節約)が、波動関数や煙密度のような
      **本物の状態は縮まない**——これは表現を変えたことの代償ではなく、
      「時間発展した状態を厳密に持つ」ことそのものの費用である。
      同じ緩和は3Dの `smoke_density`(煙が無い格子)にも効く余地があるが、
      現存シーンは全て煙を持つため入れていない。
    - **`float_roundtrip` はまだ外せない**(`raw_bytes` の当初の見込みを訂正)。
      移行したのは11ドメインの `raw_state` だけで、`bodies[].position`・
      `materials`・`thermal`・`circuit` は今も10進の f64 であり、
      D24車の save→load→60step のハッシュ一致はそのパーサ挙動に依存している。
    - 撤去に伴い `RopeJson`/`SoftParticleJson`/`SphBlockJson`/`GaussianPacketJson`/
      `Potential1dJson`/`GaussianPacket2dJson`/`DoubleSlitJson`/`GridSolidJson`/
      `GridSolid3DJson`/`SmokeBlockJson`/`FdtdInitialJson` が参照ゼロになったので
      削除した。後方互換テスト
      `scenes_without_raw_state_still_load_via_construction_recipe` は主張する
      対象そのものが消えたため削除し、代わりに「`raw_state` 無しは
      スキーマエラーになる」を固定する `domain_sections_without_raw_state_are_rejected`
      を置いた。
...
- [x] 3D CADモデリング(スケッチ・押し出し・ブーリアン)を実装する
  着手条件だった「凸分解の欠如」は近似凸分解(`sim_mechanics::decompose`、
  `Shape::from_triangle_mesh`)の実装で解消済み。ただしその時点では
  **任意の三角形メッシュを作る手段がエディタ側に無く**、`from_triangle_mesh`
  がシーンJSONからもwasm境界からも到達不能なままだった。ここを埋めた。
  - `sim_mechanics::sketch`(新規): 2D多角形ブーリアン(和/差/積、
    Weiler–Atherton系)・耳刈り三角形分割(穴はブリッジ辺で外形へ接続)・
    角柱への押し出し。すべて自前実装(外部クレート実質ゼロ)。
  - **3Dメッシュ同士のCSGは実装しない**——退化配置の処理が自前実装で
    堅牢性を担保できる規模を超えるため。合成は**押し出し前の2D断面**に
    対してのみ行う(CADの標準的な作図手順そのもので、角柱に限れば
    L字・コの字・穴あき板といった実用的な非凸形状はすべて作れる)。
  - シーンJSONに形状タグ`mesh`(`vertices`+`triangles`)を新設し、
    `shape_json_to_shape`が`Shape::from_triangle_mesh`へ流す
    (=`spawn_shape_json`からも同じ経路で到達できる)。**入力専用のタグ**で、
    書き戻しは分解後の実物(`Compound`/`ConvexMesh`)になる。
  - エディタ: ツール`S スケッチ`(W/E/R/Qと同じ排他トグル)+ Scene View の
    スケッチパネル。構築平面は既存スポーンパレットと同じ地面 y=0 で、
    グリッドスナップも共有する。
  - 検証: Rust側新規テスト20本(ブーリアン3演算の面積・穴の生成・耳刈りの
    三角形数と面積・押し出し体積・凹形状が実際に凸分解されること・
    範囲外indexでパニックしないこと)、cargo test --workspace全緑、
    fmt/clippyクリーン。Playwrightでスケッチ→減算→押し出し→スポーンの
    実UI経路を確認(スモーク34/34)。
- [ ] 汎用ECS/プラグイン機構の要否を再評価する
  着手条件: ドキュメント+スキーマ方式(現行方針)では表現できない要件が
  具体的に出てきた時点。それまでは「要るのはドキュメントとスキーマだけ」の
  判断を維持する。
- [x] Unity機能パリティ相当の追加機能を再評価する(2026-08-18、D3「フルパリティを
  目指す」決定を受けて着手)
  **着手条件の上書き**: 元の着手条件(「複雑なシナリオを組んで検証できる」という
  目標に対して具体的に不足している機能がユーザーから指摘された時点)は、構造化
  レビューでのユーザーの明示的な設計決定(D3)によって上書きされた——
  「ユーザーからの具体的な指摘を待つ」ではなく「エディタUXのギャップを能動的に
  埋めよ」という指示。ただし文字どおりの「フルパリティ」は無限定(アセット
  パイプライン・アニメーションタイムライン・音声・スクリプティング層・地形編集・
  パーティクル・ネットワーク対戦等を持つ汎用ゲームエンジン全体を1増分で再現する
  ことは非現実的かつ本プロジェクトの性質——物理**シナリオ**エディタであって汎用
  ゲームエンジンではない——と合わない)ため、**このプロジェクトのドメイン
  (剛体・ジョイント・結合・場・プローブの配置/設定)に意味のある範囲**へ
  スコープした。

  **監査結果——監査せずに「無い」と決めつけない**: 着手前に`demo/src/main.ts`
  (8000行超)を実地で確認し、既に実装済みだったものが多数見つかった
  (すべて設計docs/23-frontend/01-editor.md記載どおり):
  - **Redo**: Undo/Redoは2本のスタック(`editUndoStack`/`editRedoStack`)で
    双方向とも実装済み(Ctrl+Z/Shift+Ctrl+Z/Ctrl+Y、ToolbarのRedoボタン)。
  - **Hierarchyの複数選択トグル**: Ctrl/Cmdクリックでの追加選択・解除
    (`hierarchyMultiSelection`)は実装済み——ただし**標準の対になる操作である
    Shift範囲選択・全選択・選択解除は無かった**(下記「閉じたギャップ」参照)。
  - **Gizmoスナップのトグル/設定**: グリッドスナップ幅入力(既定10cm、0で
    無効化)は実装済み。
  - **Gizmoの Local/World 座標系切替**: `X`キー+Settingsのボタンで実装済み。
  - **Console/logパネル**: All/Errors/Warnings/Info/Contacts/Eventsの6タブ・
    イベント行クリックでTimeline/Scene Viewへ連動、いずれも実装済み——
    ただし**Errorsタブは`drain_events_text`が"errors"レベルを一度も出さず
    実質的に常に空**という具体的な欠落が見つかった(下記「閉じたギャップ」参照)。
  - **コピー/複製のキーボード操作**: 右クリックの「複製」とは別に`Ctrl+D`
    ショートカットが実装済み(候補に挙げた「Ctrl+C/Ctrl+Vによる複製」が
    求める体験は実質的にこれで満たされている)。
  - **「ここから再生」vs常に最初から**: 設計どおり、Playは常に現在のEdit
    モードの編集状態から始まる(常に最初からシーンを作り直す動作にはなって
    いない)——確認済みで対応不要。

  **閉じたギャップ(4件、いずれも既存の複数選択・Console機構の自然な延長で
  スコープが明確、新しいドメイン概念の発明を要さない)**:
  1. **Hierarchyの検索/絞り込みボックス**(`#hierarchy-search`)。ボディ数が
     多いシーン(D8散乱=51体)で目的の行を探す手段が無かった。セクションごとの
     専用ロジックを増やさない汎用フィルタ(`<li>`を舐めて自身のテキスト or
     子孫の一致で表示/非表示)として実装、折り畳まれた枝も自動的に開く
     (`#hierarchy-tree.hierarchy-filtering .tree-nested`のCSS override)。
  2. **Shiftクリックの範囲選択**(`hierarchyRangeAnchor`)。Ctrl/Cmdクリックの
     トグルはあったが、直前にクリックした行から今クリックした行までの一括選択
     という標準の対になる操作が無かった。
  3. **Ctrl+A(全選択)/Escape(複数選択の解除)**。範囲選択・トグルと対になる
     標準操作。Escapeは既存のコンテキストメニューを閉じる`Escape`ハンドラと
     独立に共存する(同じキー押下で両方が意味を持つ、競合ではない)。
  4. **Consoleの Errors タブへの失敗の永続表示**。Add Joint/Add Coupling・
     シーン読み込み・押し出し・材料派生などの失敗はこれまで`window.alert`の
     一度きりのモーダルでしか伝わらず、閉じた瞬間に消えて後から見返せな
     かった——ConsoleのErrorsタブは存在するのに常に空という、UIの見た目と
     実挙動が食い違う具体的な欠陥だった。`window.alert`自体は即時フィード
     バックとして有用なので残し、`reportError()`ヘルパーへ集約して
     **同じメッセージをConsoleへも書く**形で埋めた(呼び出し箇所20箇所を
     機械的に置換、関数シグネチャの変更なし——`main()`内で`setUpConsole`が
     返す`append`をモジュールスコープの変数へ後から配線する、既存の
     `JumpToStepRef`等と同じ「world/Console構築より前に定義される関数から
     呼べるようにする」構成)。

  **見送った項目(ドメイン内だが今回は対象外、理由つき)**:
  - **Inspectorのロック(ピン留め)**: Unityの「Inspectorを特定オブジェクトへ
    固定し、他を選択してもInspectorだけ変わらない」機能に相当。ドメイン的には
    有用(2体を見比べながら編集する用途)だが、`selectedBodyIndex`が
    Gizmoドラッグ・Motor Arm・Thrust・毎フレームのTransform更新など
    `setUpSceneView`(8000行超の1関数)全域で単一の変数として密結合しており、
    「Scene View/Gizmoの選択」と「Inspectorが表示する対象」を分離するには
    それらの参照箇所すべてを洗い出す相応の改修が要る。他の3件と比べて
    見積もりの不確実性が高く、「1つのレビュー可能な増分」に収まらないリスクが
    あるため今回は見送った——次の増分の候補として明記しておく。
  - **カメラ視点のブックマーク(視点の保存/復元)**: ドメイン的には有用だが、
    Timelineの時刻ブックマーク機能と概念が近く(「後で戻れる場所に印を付ける」)
    実利用上の優先度は上記4件より低いと判断し、今回は見送った。
  - **Inspector以外の全般的なロック機構、複数オブジェクトの一括プロパティ編集
    (Multi-object editing)**: 監査時点でUIに具体的な着手条件(複数選択自体は
    今回強化したが、Inspectorが単一選択前提なのは設計どおりの縮約——設計
    docs/23-frontend/01-editor.md §1.1「複数選択は Hierarchy 上の操作対象の
    集合として機能する」)が無く、範囲が広がりすぎるため対象外。

  **恒久的にスコープ外(ドメイン外、汎用ゲームエンジンの機能であって物理
  シナリオエディタには不要)**: アセットインポートパイプライン・アニメーション
  タイムライン・音声ミキシング・C#等のスクリプティング層・地形彫刻ツール・
  パーティクルシステム・ネットワーク対戦。これらは今後も「Unity機能パリティ」を
  理由に着手しない——本エディタが対象とするのは剛体・ジョイント・結合・場・
  プローブの配置/設定・検証であり、汎用ゲームエンジン相当を目指すものではない。

  検証: Rust側は無変更(TypeScript/HTML/CSSのみの増分、`cargo test --workspace`・
  `cargo fmt --all -- --check`・`cargo clippy --workspace --all-targets -- -D
  warnings`はいずれも無変更のまま緑であることを確認)。`npm run build`
  (`tsc --noEmit && vite build`)緑。`wasm-pack build`でwasm実ビルドを確認した
  うえで、Playwright e2e(`npm run test:e2e`)を実行——既存34本+新規2本の
  計36本全緑。新規テストは①Hierarchy検索(D8散乱51体で"floor"→1件・"s1"→
  11件〈s1本体+s10〜s19〉に絞り込まれること)・Shift範囲選択(3件選択の
  「複製 (3件)」表示)・Ctrl+A(51件選択)・Escape(単一選択へ戻ることを
  「複製」の件数サフィックス消失で確認)、②Validationタブの入力検証失敗が
  `window.alert`とConsole Errorsタブの両方に現れること、の2本。
...
- [x] 条件付き項目4件の着手可否を評価する
  ...
  3. **Unity機能パリティ**: 未着手。ユーザーから具体的な不足機能の指摘は
     無い。
     **2026-08-18追記**: この着手条件(ユーザーからの具体的な不足機能の
     指摘)を経ずに、ユーザーの明示的な設計決定(D3「フルパリティを
     目指す」)が着手条件そのものを上書きした。監査結果・実装内容・
     見送った項目は上の「Unity機能パリティ相当の追加機能を再評価する」
     (2026-08-18)を参照。
...
- [x] 重力を`GravityField`(一様/点源/無重力)へ抽象化する
  ...
  **一様場しか表せない**——空間のどこでも同じ加速度ベクトルになる場である。
  ...
  - **正直な適用範囲(縮約)**: 非`Uniform`な場では`gravity()`が0.0を返す。
    ...
    本増分では踏み込んでいない(→ 後続の「浮力を重力場へ追従させる」で
    浮力とBoussinesq浮力については解消済み。自然対流のレイリー数だけが
    引き続き`gravity()`を読む)。
...
- [x] 浮力を重力場(`GravityField`)へ追従させる
  上の`GravityField`増分が「別の計画作業」として送った積み残しの回収。
  `sim_fluid::buoyancy`は水面を**ワールドy座標の水平面**として持ち、浮力を
  常に`+y`向きに出していた——つまり「重力の向きはワールド`-y`」が暗黙の前提で、
  重力の向きを変えても浮力だけが元の鉛直を向き続け、非`Uniform`な場では
  `gravity()`が0.0を返して浮力が丸ごと消えていた。
  - **何が「重力に追従する」ことになったか**: 剛体位置における局所的な重力
    (新設の`GravityField::up_and_magnitude_at(position)`)から
    「上」$\mathbf{u}$と大きさ$|g|$を取り、(1)自由表面を
    $\mathbf{x}\cdot\mathbf{u} = \texttt{water\_level}$ の平面として水面下体積を
    切り(新設の`sim_fluid::submerged_box_below_plane`)、(2)浮力を
    $\rho_f|g|V_\text{sub}\,\mathbf{u}$ として出す(`buoyancy_force`が`up`を
    引数に取る形へ)。`water_level`の意味は移行前と同じ「原点から`up`軸に
    沿って測った符号付き距離」で、`up=(0,1,0)`なら「水面のワールドy座標」に
    一致する。
  - **傾いた水面の切断は近似ではなく厳密**: 直方体を半空間で切る
    (6面をSutherland–Hodgmanで切り、切断面にフタを張って発散定理で
    体積・重心を出す)。`up=(0,1,0)`のときだけ移行前の閉形式へ委譲する分岐を
    残してある——**モデルの違いではなく丸めの経路を移行前のまま保つため**で、
    両者が同じ量を返すことはテストで固定した。
  - **3つの経路すべてを揃えた**: 埋め込み経路(`MechanicsSolver::apply_forces`
    の`fluids`処理)・結合登録経路(`sim_coupling::BuoyancyDrag`)・
    Boussinesq浮力(`sim_coupling::BoussinesqBuoyancy`)。片方だけ直して
    二つの実装が食い違う状態にしない。
  - **`Zero`場では浮力は厳密に0**(`up_and_magnitude_at`が`None`)。「下」が
    無い場所に水平な自由表面は定義できず、押しのけた流体の重量も0なので
    物理的にも正しい——向きを発明して力を出すより、消えることを明示する。
  - **`PointSource`場でも浮力は効く**(剛体側のみ)。局所的な「上」は中心から
    外向きで一意に定まるので、同じ式がそのまま使える。**残る縮約**: 自由表面を
    等ポテンシャル面(球面)ではなく局所鉛直に垂直な**平面**で近似する
    (剛体の寸法が水域の曲率半径に対して小さい範囲の近似)。
  - **Boussinesq浮力は一様場のみ**(`PointSource`では無効のまま、移行前と同じ)。
    `GridFluid2D`はワールド座標系での位置を持たないので、位置依存の場を
    評価する点そのものが無い——代表点を発明するより効かないことを明示する。
    一様場では向きに追従し、水平成分は`u`へ、鉛直成分は`v`へ入る(格子の軸が
    ワールドx/y軸に対応するのは`GridFluidRigid`と同じ既定の解釈)。
    重力のz成分は2D格子に対応する速度成分が無いので落ちる。
  - **追従しないまま残したもの**: (1)自然対流の熱伝達率
    (`sim_coupling::ConvectionLink`)——「一様な鉛直重力」を前提とする経験
    相関式なので、スカラー縮約`gravity()`を読んだままにした。(2)**剛体の姿勢**
    ——水面が傾いても切られる直方体はワールド軸に平行なままで、浮心が`up`から
    横へずれて生じる復原モーメントも積んでいない(一般姿勢の凸多面体切断は
    設計 docs/11-fluid/04 §4 のまま未着手)。
  - **力の積分方式は変えていない**: `BuoyancyDrag::apply_pre`は群5の時点で
    既に「集めた力をその場で$v \mathrel{+}= F\,\Delta t/m$と速度へ入れる」
    直接キック(半陰的Eulerの速度更新と同じ相・同じ形、`apply_post`は
    二重計上を避けるため空)になっており、本増分で触る余地は無かった。
    向きだけを正した。
  - 検証: `sim_fluid`に切断の解析解テスト(中心を通る平面はどんな向きでも
    箱を半分にする・傾いた水面が側面を横切る帯では断面積が一定・全没/完全露出の
    飽和・速い経路と一般経路の一致)と、独立な数値積分(細格子の占有率)との
    突き合わせを追加。`sim_coupling`に「重力を30°傾けると浮体が**傾いた`up`軸に
    沿って**アルキメデスの閉形式どおりの喫水で釣り合う」単振動テスト(平衡位置は
    ワールドxにもずれる=`+y`固定の実装では出せない)、`Zero`で浮力が厳密に0、
