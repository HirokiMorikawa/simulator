...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  F10(ダム崩壊 vs Martin-Moyce実測)はMartin & Moyce 1952の信頼できる実測データを
  入手できず未実装のまま(詳細は§8のF10注記)。CIのdebugビルドでのテスト実行時間を
  ...
  サンプリングの重なり判定一致)も実装した。EPA(貫入深さ)・フルCCDは後続増分に残す。
- **作業中**: なし(直前の増分でGJK(分離距離・重なり判定)が完了、次点候補から自由に選択)。
  次点候補: EPA(貫入深さ)・フルCCD(Phase 5)、イジングL=256フル版(長時間級)
...
- [x] WCSPH(境界粒子・剛体双方向結合は静的境界のみ、動的結合は未実装)—
      ...
      で検証する。F10(ダム崩壊先端 vs Martin & Moyce 1952実測)は信頼できる実測データを
      入手できず未実装
      (下記F10注記参照)
...
- [x] イジング(2D、$h=0$、L=256フル版は長時間級のため未実行、L=64縮約のみ)—
      ...
      形状に直った
- [x] GJK(分離距離・重なり判定のみ。EPA(貫入深さ)・フルCCDは未実装)—
      `crates/sim-mechanics/src/gjk.rs::{gjk_distance, ConvexShape, GjkResult}`。ミンコフスキー差
      の凸包に対する原点への最近点をJohnsonのサブアルゴリズム(単体の全部分集合を試し、
      原点の重心座標が非負になる部分集合のうち最近のものを採る素直な実装、設計§4.5の
      「実装の要諦は書籍を正とする」を受けて教科書の完全な実装ではなくこの方式にした)で
      反復探索。分離2球・重なり2球・分離した2つの箱(8頂点の点群)で解析解と一致することを
      確認し、加えて設計§4.5が推奨する統計テスト(乱数配置(決定シード)の凸四面体対で
      GJKの重なり判定と総当たりサンプリングが一致)も実装。EPA(重なり時の貫入深さ・法線
      復元)とフルCCD(GJKと同時に実装するconservative advancement)は後続増分で追加する
...
- [ ] レジーム切替(時間加速)
- [x] 1PN 補正(オプトイン、A8・A9のみ。`RelativitySettings`構造体・`NBodySystem`への
      完全統合・光の曲がり(A10)・J2摂動(A5)・大気減衰(A6)は未実装)—
      `crates/sim-astro/src/relativity.rs::{pn1_acceleration, pn1_precession_per_orbit,
      gps_proper_time_rate}`。A9(GPS固有時率、設計§2.2)は解析式のみ(シミュレーション
      不要)で+38.6μs/日にrel<1%で一致。A8(近日点移動、設計§2.1のSchwarzschild項)は、
      実際の太陽・水星のGM/c²比では43″/世紀という極小の歳差を検出するのに非現実的な
      数の周回積分が要るため、GM/c²比を誇張した二体系(主星固定・test-particle近似)で
      少数周回積分し、同じ誇張パラメータでの解析式$\Delta\varpi=6\pi GM/(c^2a(1-e^2))$と
      比較する方式にした。実装検証中、誇張しすぎる(c=20相当)と解析式(1PNの線形近似)
      からの系統的なずれが大きくなる(rel_err≈14%、ステップ数を増やしても縮まらないため
      数値誤差ではない)ことを発見し、誤差がGM/c²にほぼ比例して縮小する挙動から、線形の
      1PN近似自体が過度に強い摂動では破れる(2次以降の項が無視できなくなる)ことが原因と
      判明。誇張を弱めることでrel<1%を達成した
- [ ] 担当テスト Green: A1–A10(A1・A2縮約版・A3・A4・A7・A8・A9 Green。A5・A6・A10は
      J2摂動/大気減衰/光の曲がり未実装)
...
> **F10 注記(実装時確認)**: Martin & Moyce 1952 の実測ダム崩壊先端位置データを信頼できる形で
> 入手できなかったため、文字通りのF10比較は実装していない。代わりにRitterの古典解析解
> (無限貯水池仮定、先端速度$2\sqrt{gH}$一定)との比較を試みたが、有限長の貯水槽では
> 先端速度が単調に上昇し続け(実測~0.64→2.12m/s)、無限貯水池前提のRitter解とは前提が
> 異なり比較にならないと判明した。設計§7が挙げる他の実測データ非依存の検証項目
> (全運動量保存・静水圧平衡)をWCSPH(`crates/sim-fluid/src/sph.rs`)側で実装・Green化する
> ことで代替し、F10自体は実測データ入手待ちとして未着手のまま記録する。
