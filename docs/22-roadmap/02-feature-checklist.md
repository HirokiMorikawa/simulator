...
- フロントエンドは6パネルドッキングレイアウト(Scene View/Hierarchy/Inspector/
  ...
  (D1/D2/D3/D4/D5(静止+滑走)/D6(静止+振動)/D7(高Re+低Re)/D11/D21/D26の
  13シーンをJSONアセット化、`loadScene`で切替可能)まで実装済み。

**カウント(本増分後の実測値)**: チェックボックス総数 **262** / 済 **209** / 未 **53**
(完了率 79.8%)。節別内訳:
...
| §7 デモ合格管理表 | — | — | 33 | うち25件が「ヘッドレスGreen、目視チェック保留」で同一理由滞留(本増分でD1/D2/D3/D7/D21/D26の6件を目視チェック完了)。D24は新規物理待ちでスコープ外明記 |
...
| B | シーンギャラリー拡張(シーン定義プローブのProbe Graphs配線 + 力学/熱/天体デモのシーン切り出し)。B1(Probe Graphs配線)完了済み、B2(力学デモ8件のシーン切り出し: D1/D2/D3/D5滑走/D6振動/D7高低Re/D21/D26)完了済み、熱/天体デモ(D9/D34/D35等)のシーン切り出しは後続の増分へ | §7の多数(「ヘッドレスGreen、目視チェック保留」で滞留する25件が主対象) |
