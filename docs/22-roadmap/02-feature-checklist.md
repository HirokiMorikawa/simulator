...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  A2(縮約版)・A3・A7・A1(8合成衛星でのケプラー第3法則、公転周期は線形補間ゼロ交差で
  実測)を Green 化 — 力学以外のドメインへの展開第一弾)
- **作業中**: 力学(`sim-mechanics`)P1 最後の残り — 最小CCD(M15、意図的に後回し。
  下記 §2/§3 の P1 行)。次点候補: split impulse(M12 Green化に必要)、
  他ドメイン(em/quantum/statistical)の Phase A スケルトン
