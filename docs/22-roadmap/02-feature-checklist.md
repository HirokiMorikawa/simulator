...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  S4・S5 を Green 化(S4 実装検証中に BAOAB の位置更新離散化誤差が γΔt/m に強く依存する
  ことを発見・文書化)。`sim-em` に静電場(`PointChargeSystem`、点電荷直接和クーロン力 +
  一様外場合成 + Boris pusher 積分)を実装し E1・E2 を Green 化(E2 は既存の
...
- **作業中**: 力学(`sim-mechanics`)P1 最後の残り — 最小CCD(M15、意図的に後回し)。
  ...
  S6(沈降平衡)、磁気双極子・回路MNA(sim-em 残り)、他ドメイン(quantum)の Phase A スケルトン
...
- [ ] 担当テスト Green: E1–E7, E9–E12, F10, S4–S6, T8(E1・E2・S4・S5 Green。S6=沈降平衡は
      境界条件付きヒストグラム測定が必要で未実装)
