...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  全運動量保存(機械精度)と静水圧平衡(圧力p=ρgh、実測rel<15%)をGreen化。F10
  (ダム崩壊 vs Martin-Moyce実測)はMartin & Moyce 1952の信頼できる実測データを入手できず
  未実装のまま(詳細は§8のF10注記)。
- **作業中**: なし(直前の増分でWCSPHが完了、次点候補から自由に選択)。
  次点候補: GJK/EPA・フルCCD(Phase 5)、FDTD(E8/E13)、イジングL=256フル版(長時間級)、
  車両(Pacejka、P4残り)
...
- [x] WCSPH(境界粒子・剛体双方向結合は静的境界のみ、動的結合は未実装)—
      ...
      ではなく、上記近似・人工音速による弱圧縮性・有限の人工粘性による残留振動を踏まえた
      実測で安定的に達成できる誤差域(rel<15%、実測は概ね5–9%)で検証する。F10
      (ダム崩壊先端 vs Martin & Moyce 1952実測)は信頼できる実測データを入手できず未実装
      (下記F10注記参照)
- [ ] 車両(Pacejka)
...
- [x] イジング(2D、$h=0$、L=256フル版は長時間級のため未実行、L=64縮約のみ)—
      ...
      形状に直った
- [ ] GJK / EPA・フル CCD
- [ ] 担当テスト Green: Q1–Q6, E8, E13, S1–S3, S7–S9(Q1・Q2・Q3・Q4・Q5・Q6・S1・S2・S3・
      S7・S8・S9 Green。残るE8・E13はFDTD)
