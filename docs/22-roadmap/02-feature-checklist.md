...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  重い検証のため後続増分に残す。
- **作業中**: なし(直前の増分でX1が完了、次点候補から自由に選択)。
  次点候補: 格子流体の残り(P3、F7・F11、固体境界の実装が前提)、X2(格子流体×剛体、
  重い検証)、イジングL=256フル版(長時間級のため優先度低)、Phase A型スケルトン
...
- [ ] 担当テスト Green: M3, M4, M13, M14, F7–F9, F11, T3, T5, T7(M3・M4・M13・M14・F8・F9・T3・
      T5・T6・T7 全てGreen。残るはF7(固体境界+収束次数)・F11(円柱障害物+渦度強化判断))
...
- [ ] F7(固体境界(ポアズイユ流の壁面)+4解像度収束次数検証が必要、後続増分に残す)
