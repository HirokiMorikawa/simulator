...
| 7 | 場のソルバの縮約解消(FDTD PML・ConductionRod ρc_p/可変伝導率・回路フォールバック連鎖/モータ) | **完了**(GridFluid流入出・3D化は残置、下記) |
...
- **残置**: GridFluid2D の流入出/固体境界/3D、ConductionRod の3D(7点ステンシル)。
  どちらも既存の解析解テスト(F8 Taylor-Green・F9 投影後発散・T3 フーリエ級数)が
  依存している中核ソルバの構造変更(圧力ポアソンの境界条件・格子の保持方法)を
  伴うため、独立した増分として切り出す。
