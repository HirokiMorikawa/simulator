...
- **フェーズ**: 設計(レビュー対応)
- **作業中**: 対策方針書([../reviews/pr1-review-countermeasures.md](../reviews/pr1-review-countermeasures.md)、PR #4)のレビュー対応
- **次**: 改訂 PR-1〜PR-7(下記 §0)
...
- [ ] C-1 決定論水準の決定(案 1 緩和 / 案 2 全面維持)
- [ ] C-3 位置表現(保持フレーム)の決定
- [ ] C-5 最小 CCD の方式選定(speculative contact / ray-cast bullet)

改訂 PR:

- [ ] PR-1 粒度規約・R-1 運用ルール・対応不要判断の反映(vision §4、phases.md 横断ルール)
- [ ] PR-2 テスト表の実装可能性監査・長時間級ルール・Boris pusher・Wolff 必須化・F11 注記
- [ ] PR-3 決定論方針の反映・台帳再定義・sub-iteration 規則 + stiff 検出テスト行
- [ ] PR-4 位置表現の決定反映・最小 CCD 標準機能化 + 検証テスト行・立位保持基準の置換
- [ ] PR-5 性能構成規則・wasm 配布戦略・巻き戻しコスト
...
- [ ] 摩擦(クーロン・摩擦円錐)
- [ ] 最小 CCD(弾丸級の speculative contact — 対策 C-5)
- [ ] 位置表現 = フレーム ID + ローカル座標(対策 C-3 決定の実装)
- [ ] 重力・抗力・浮力
- [ ] 熱ノード(基礎)
- [ ] エネルギー台帳(残差トレンド監視 — 対策 C-2)
- [ ] 担当テスト Green: M1–M9, M12, F1–F6, T1, T2
...
- [ ] 並列リダクション(順序固定 — C-1 決定に従う)
- [ ] 担当テスト Green: M3, M4, M13, M14, F7–F9, F11, T3, T5, T7

### P4 — 電磁・光・SPH・車両・ブラウン

- [ ] 回路(MNA・非線形素子収束)
- [ ] モーター結合(sub-iteration 決定的算出 — 対策 C-6)
- [ ] 静電場・静磁場
- [ ] 幾何光学
- [ ] WCSPH
- [ ] 車両(Pacejka)
- [ ] ランジュバン(ブラウン運動)
- [ ] エンティティ受け入れ: 関節 PD 静的姿勢維持(置換基準 — 対策 R-3)
...
- [ ] FDTD(Yee・PML)
- [ ] 気体分子運動
- [ ] イジング(Metropolis + Wolff 必須 — 対策 R-5)
- [ ] GJK / EPA・フル CCD
...
- [ ] 軌道・再突入・宇宙機
- [ ] フレーム階層・floating origin(詳細設計文書どおり — 対策 C-3)
- [ ] レジーム切替(時間加速 — 対策 C-4)
- [ ] 1PN 補正(オプトイン)
- [ ] 担当テスト Green: A1–A10
...
- [ ] 統合シナリオ: 断熱圧縮
- [ ] 統合シナリオ: 再突入
- [ ] CI ゲート: 決定論(2 回実行一致・スナップショット再開一致・並列=逐次は C-1 決定に従う)
- [ ] CI ゲート: 保存則 residual
- [ ] CI ゲート: 性能ベンチ回帰(構成規則 — 対策 R-4)
