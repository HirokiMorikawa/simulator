...
- **ワークストリームD(フロントエンド)**: Phase 0スタブから、6パネルドッキングレイアウト
  ...
  3点を`body_pos_y`/`body_speed`の2プローブだけから導出して検証した。
...
- [ ] 回路エディタ(Scene View サブモード、D19 自由配線。Project ドロワーに
      ...
      本増分は固定回路の可視化パネルのみ)
- [ ] フレームサブモード(L5 ドリルイン。フレーム軸オーバーレイ(単一の自転
      フレームの可視化、上記オーバーレイ項目参照)は実装済みだが、複数フレーム
      の階層ドリルイン・切替UIは未実装)
...
- [ ] D6 浮き沈み(ヘッドレステストGreen、同上。目視チェック保留。F4(密度比0.6の
      ...
      **シーンJSON経由の3本目の適用例(本増分で追加)**: F4部分を
      ...
      から大きくずれない(安定平衡)ことを確認。F5(振動周期)はゼロ交差検出
      にはシーンJSON側の追加ロジックが要るため対象外)
