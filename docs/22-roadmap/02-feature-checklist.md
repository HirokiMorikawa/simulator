...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green — 線形代数・PRNG・
  ...
  `Solver`トレイト・`MaterialDb`・`EventQueue`・`FrameId`。`sim-mechanics` の `RigidBodySet`/`Shape`
  (Sphere/Box/Plane)/`MechanicsSolver` を実装し、重力下の semi-implicit Euler 積分で M1(自由落下)を
  Green 化。`sim-world` の `World` を Phase 0 の `FallingBody` から `MechanicsSolver` 経由に移行)
- **作業中**: 力学(`sim-mechanics`)の残り — 衝突検出(broadphase/narrowphase)・接触ソルバ・摩擦・
  最小CCD・重力以外の力(抗力/浮力)・熱ノード基礎(下記 §2/§3 の P1 行)
...
- [x] 位置表現 = フレーム ID + ローカル座標(`sim_core::FrameId`、単一ルートフレームで運用中。
      フル階層は Pα)
- [ ] 重力・抗力・浮力(重力は実装済み。抗力・浮力は流体結合待ち)
- [ ] 熱ノード(基礎)
- [ ] エネルギー台帳(残差トレンド監視)
- [ ] 担当テスト Green: M1–M9, M12, M15, F1–F6, T1, T2(M1 のみ Green。残りは衝突/接触/摩擦/CCD/
      熱ノード実装後)
