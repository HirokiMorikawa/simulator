...
**カウント(本増分後の実測値)**: チェックボックス総数 **262** / 済 **262** / 未 **0**
(完了率 **100%**)。節別内訳:

| 節 | 総数 | 済 | 未 | 未チェック項目の性質 |
|---|---:|---:|---:|---|
| §2 Phase A | 8 | 8 | 0 | 各設計文書§7とのユニットテスト網羅監査(未実施のまま正直に保持) |
...
| §6 フロントエンド | — | — | 0 | Console連動、Inspector Component、Toolbarシーン選択等。**Probe対数軸/CSV(E1)・HierarchyのProbes(E2)・レイアウト4種+ドロワー開閉(E3)・Hierarchyの Circuits(G2、回路素子の列挙APIを新設)は完了** |
...
| B | シーンギャラリー拡張(シーン定義プローブのProbe Graphs配線 + 力学/熱/天体デモのシーン切り出し)。B1(Probe Graphs配線)完了済み、B2(力学デモ8件のシーン切り出し: D1/D2/D3/D5滑走/D6振動/D7高低Re/D21/D26)完了済み、B3(D9/D34/D35のシーン切り出し+「ボディ0個のシーン」対応)完了済み。D25(ブラウン運動、300粒子を`format!`で動的生成)は静的ファイル化に不向きなためインラインのまま維持(§7 D25の行に理由を明記)。**増分G1でD8/D12/D36を追加**——ここでB3時点の「D36–D39は現行`Scenario::astro`スキーマでは表現できない」という記述の**うちD36の分は誤りだったことが判明した**。双曲線軌道は初期位置・初期速度だけで決まるので既存スキーマでそのまま書ける(専用の軌道要素指定は要らない)。残るD37/D38/D39は大気抗力・潮汐力場・相対論補正というスキーマ外の設定を要するため対象外のまま | §7の多数(「ヘッドレスGreen、目視チェック保留」で滞留していた件) |
| C | Phase D画像出力パイプライン(フレームバッファ+自前PNG出力+カメラ接続、露出、モーションブラー、MaterialDbブリッジ、コースティクス)。**C1(フレームバッファ+自前PNG出力+カメラ接続)・C2(露出のEV換算)・C3(モーションブラー)完了済み** | §5の3件・§7のD40–D43・§8のR3 |
...
| 6 | レンダラの縮約解消(NEE+MIS・ロシアンルーレット・三角形メッシュ・PNG deflate・コースティクス合成/分光) | **完了**(SAH分割は残置、下記) |
| 7 | 場のソルバの縮約解消(FDTD PML・ConductionRod ρc_p/可変伝導率・GridFluid流入出・回路) | **進行中**(FDTD PML と ConductionRod 完了、下記) |
...
- **ConductionRod1D の $\rho c_p$ と可変熱伝導率**(設計 docs/12-thermal/02 §4.3
  ...
  **実装検証中に踏んだ2点**: ①界面は格子点ではなく**面**$(mid-\tfrac12)\Delta x$に
  ...
  dtを桁ごと上げるのが正しい対処。
- **残り**: GridFluid2D の流入出/固体境界/3D、回路の Newton フォールバック連鎖と
  モータ素子、ConductionRod の3D(7点ステンシル)。
...
- **残置**: SAH分割(BVHは最長軸中央値のまま)・完全な分光レンダリング本体の
...
| Q4 | `sim-mechanics/src/collision.rs:6-8` | マニフォールド持続化が無く多段スタックの貫入がslopを超える | **検証の上、既知の限界として維持**(下記参照、M12は現状合格だが余裕僅少) |
