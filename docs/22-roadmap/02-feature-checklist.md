...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  として記録。`sim-astro` に N体重力(総当たり+leapfrog、`nbody.rs::NBodySystem`)を実装し
  A2(縮約版)・A3・A7 を Green 化 — 力学以外のドメインへの展開第一弾)
- **作業中**: 力学(`sim-mechanics`)P1 最後の残り — 最小CCD(M15、意図的に後回し。
  下記 §2/§3 の P1 行)。次点候補: split impulse(M12 Green化に必要)、`sim-astro` の
  太陽系8惑星デモ(A1)、他ドメイン(em/quantum/statistical)の Phase A スケルトン
...
- [ ] レジーム切替(時間加速)
- [ ] 1PN 補正(オプトイン)
- [ ] 担当テスト Green: A1–A10(A2縮約版・A3・A7 Green。A1=太陽系8惑星デモ待ち、
      A4–A6・A8–A10=軌道力学/1PN未実装)
...
- [ ] A1(太陽系8惑星デモ待ち。ケプラー第3法則自体は2体系で検証可能だが設計が要求する
      「8惑星」規模のデモ構築が必要)
