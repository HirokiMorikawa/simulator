...
- **作業中**: ワークストリームA(Phase B残タスク)継続中 — 次はX2(格子流体×剛体)。
- **次**: A(残り: X2・フレーム階層・レジーム切替・エンティティ関節PD・F10)→ B(Phase C:
...
- [ ] 流体 F1–F11 を記述、全 Red 確認(F1–F9・F11はGreen。F10はMartin & Moyce 1952実測
      データ入手待ちのまま未着手。§8参照)
