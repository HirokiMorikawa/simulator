...
- フロントエンドは6パネルドッキングレイアウト(Scene View/Hierarchy/Inspector/
  ...
  (D4/D5/D6/D11の4シーンをJSONアセット化、`loadScene`で切替可能)まで実装済み。

**カウント(本増分後の実測値)**: チェックボックス総数 **262** / 済 **203** / 未 **59**
(完了率 77.5%)。節別内訳:
...
| §7 デモ合格管理表 | — | — | 39 | うち31件が「ヘッドレスGreen、目視チェック保留」で同一理由滞留。D24は新規物理待ちでスコープ外明記 |
...
| B | シーンギャラリー拡張(シーン定義プローブのProbe Graphs配線 + 力学/熱/天体デモのシーン切り出し) | §7の多数(「ヘッドレスGreen、目視チェック保留」で滞留する31件が主対象) |
