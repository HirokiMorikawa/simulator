...
| 9 | **群4–群7で明示的に残置した7項目を全部やる**(マニフォールド持続化・フルCCD・GridFluid渦度強化/任意形状固体境界/3D化・PhaseChangeMorphの実体変換・BVHのSAH分割) | **完了**(7/7、下記) |
...
**群10(2026-08-04)の記録 — 3D格子流体の性能(README の既知の制約を1件つぶす)**:

群9が「設計§4.4 が言う**性能ベンチからの要求**が示された記録」として残した
64³ 795.7 ms/step(予算 4 ms の約200倍)に応えた増分。設計§4.4 の指示どおり
**マルチグリッド前処理 PCG(MGPCG)** を実装した
(`crates/sim-fluid/src/pressure_multigrid.rs`、新設)。

設計§4.4 が定めていた4点をそのまま実装した:

- **分岐条件**: 8子が solid/fluid 混在の粗セルの割合が 30% を超えたら粗格子化を採用しない。
  トポロジからの静的判定なので決定論的(実行時の残差測定に依存しない)。
- **係数平均法**: 混在セルの粗格子係数は子セルの流体体積率で平均する。
- **Dirichlet の支配性**: 8子に1つでも Dirichlet があれば粗セルも Dirichlet。
- **フォールバック**: 打ち切ったレベルを最粗として V サイクルを短縮する。1段も
  粗格子化できないときは V サイクルが減衰 Jacobi 掃引に退化する = 設計の言う
  「Jacobi 前処理 PCG への退行」がそのまま起きる。

**実装で確定させた点(設計が書いていなかったもの)**:

- 平滑化は**減衰 Jacobi**($\omega=6/7$)。Gauss–Seidel の方が1掃引あたりは強いが、
  Jacobi は掃引順序に依存しないので PCG が要求する $M=M^\top$ を**構造的に**保証できる。
  制限は延長の転置 $R=\frac18P^\top$ として書き、対称性を実装で担保した。
- 粗格子作用素は Galerkin 積($RLP$)ではなく**再離散化**。不規則境界では Galerkin 積の
  疎構造が7点に収まらず、matrix-free の利点を失うため。
- 分岐条件の分母は「そのレベルで未知数を持ちうるセル」(全子が Solid でない粗セル)。
  全域が固体の広い領域で混在率が薄まって規則が効かなくなるのを避けるため。

**実測**(`cargo run --release -p sim-fluid --example grid_fluid3d_bench`、同一マシン):

| 解像度 | 群9 | 群10 | うち圧力投影 | 投影の反復数 |
|---|---|---|---|---|
| 32³ | 68.9 ms | 24.9 ms | 11.4 ms | 7 |
| 64³ | 716.8 ms | **206.8 ms** | 91.1 ms | 7 |
| 128³ | (未測定) | 1840 ms | 935 ms | 8 |

設計§10 の「反復数を大幅削減(~$O(n)$)」は達成した——**反復数が解像度にほぼ依存しない**。
群9は 32³→64³ で時間が 10.4 倍(体積8倍に対して超線形)だったのに対し、群10は
8.3 倍・64³→128³ で 8.9 倍でほぼ線形である。

**移流も同じ増分で整理した**(352 → 110 ms @64³)。周期ラップ(整数除算)と `clamp` を
三線形補間の8点ごとに踏んでいたのを軸ごとに1度だけ解く形にしたもの。あわせて
`advect_velocity` が毎ステップ `GridFluid3D` 全体を2回 clone していたのをやめ、
面アクセスと補間の**唯一の実装**として `VelocityField` に切り出した。
**離散化は1ビットも変えていない**——2Dとの交差検証テストが通り続けることがその担保。

**予算 4 ms にはまだ約50倍届かない(正直な記録)**。群9の約200倍からは縮んだが、
設計§10 が並べた手段のうち SIMD カーネル・rayon による格子タイル並列・WebGPU が
いずれも未着手である。近似バッジは「前処理なしPCG」から
**「CPU単精度なしのMGPCG」**へ差し替え、何が残っているかを申告する内容に変えた。
粗格子化を打ち切った場合はバッジ「粗格子化の打ち切り」が追加で出る。

**検証**(`crates/sim-fluid/tests/pressure_multigrid.rs`、9本):

前処理は**解を変えてはならない**ので、検証は3層にした。
(1) レベル0の作用素が置き換え前の7点ステンシルと係数まで一致すること(物理が
動いていないことの直接の証明)、(2) $M=M^\top$ かつ正定値であることを乱数ベクトルで
機械的に確認(ここが崩れると CG の収束理論が失効する)、(3) 反復数が解像度に
ほぼ依存しないこと。加えて市松模様の固体(どう粗格子化しても全粗セルが混在)で
**分岐条件が実際に働いて1段も粗格子化しない**ことと、球のような滑らかな境界では
逆に粗格子化が通ることの両方を固定した(「常に打ち切る」だけの安全側実装で
テストが通ってしまうのを防ぐため)。

`sim_math::Preconditioner` に `Custom(&dyn Fn)` を足した。マルチグリッドは格子の
ステンシルと粗格子化の規則を知る必要があり、`sim-math` に置くべきものではないため
——差し込み口だけを `sim-math` に置き、中身はドメイン crate に持たせる形にした
(IC(0) について既に同じ方針が `pcg.rs` の doc に書いてある)。
...
> **その後(群10)**: この要求に応えて MGPCG を実装し、64³ は 206.8 ms/step になった。
> 上の数値と近似バッジ「前処理なしPCG」は群9時点の記録であって、現行の実装ではない。
...
**増分9-6(`PhaseChangeMorph` の実体変換)完了**:

設計 docs/20-integration/01-coupling-matrix.md §3「P3: 融解 → 剛体消滅/流体生成
イベント」の**後半(流体生成)**。融けた質量ぶんを SPH 粒子として実際に生成する。

- `MeltSpawn { spawn_radius, seed }` を追加。液相率の**増分**から
  $\Delta m=m_0\Delta f$ を求めて繰り越しに積み、粒子1個ぶん(`sph.mass`)溜まるたびに
  `SphFluid::add_particle` で1個生成する。**既存の熱の対記帳(設計§2規則1)と同じ構造**で
  質量も対記帳する。生成位置は剛体中心まわり `spawn_radius` の球内(表面上の点を
  $r\sqrt[3]{u}$ で内側へ引いて一様分布)、速度は剛体の速度を引き継ぐ(運動量の連続性)。
- 決定論は `SimRng::new(seed, 生成カウンタ)` で担保(設計 docs/01-math/04-random.md §2)。
- シーンJSONへ `couplings[].phase_change_morph.melt_spawn` を公開し、
  新シーン `scenes/d18b-ice-melts-into-water.json` を追加した。

**残置理由が事実誤認だったことを訂正して記録する**。群5では
「`Coupling::apply` がイベントキューにも `World` の世代管理にも `DomainStates` 経由で
アクセスできないため対象外」と書いたが、**SPHへの注入にはそのどちらも要らなかった**
——`DomainStates` は最初から `sph: Option<&mut SphFluid>` を公開しており、
`SphFluid::add_particle` も既に存在していた。アーキテクチャ拡張は不要で、当時のまま
書ける状態だった。**実際に残る制約はこちら**: `World` のイベントキューは触れないので
設計の言う「イベント」としては発行できず、`spawned_particles()` というカウンタで
露出する(モジュールdocに明記)。

**検証**(`phase_change_morph.rs` に4本、`scenario.rs` に1本):

| 検証 | 結果 |
|---|---|
| **厳密な質量の対記帳** | 生成粒子数×粒子質量 + 剛体の残存質量 + 繰り越し = `initial_mass`(誤差 < 1e-12) |
| 量子化の余り | 常に粒子1個ぶん未満(生成漏れが無い) |
| **回帰** | `spawn: None`(既定)なら粒子は1つも生成されず、繰り越しも 0。既存2テストも無変更でGreen |
| **決定論** | 同一シードで2回走らせて粒子位置・速度が**ビット一致** |
| 生成位置・速度 | `spawn_radius` 内に入り、剛体の速度を引き継ぐ |
| シーンJSON経由(D18b) | 2000ステップで粒子が生成され、剛体は軽くなり、生成質量は剛体の失った質量を超えない |

**増分9-5(GridFluid3D を World / シーンJSON / エディタへ配線)完了 —— 群9の締め**:

9-4 で作った `GridFluid3D` を、他ドメインと同格の一級市民として繋いだ。

- `World` に `grid_fluid_3d` スロットを追加し、**固定ドメイン順序の `grid_fluid`(2D)の
  直後**に置いた。`total_energy`(SI・保存系なので合計に入れる)・`energy_report`
  (`GridFluid3D` 行)・サブステップ実行・`state_hash`・`active_approximations` の
  全経路へ既存パターンどおり接続。
- `sim_coupling::DomainStates` にも `grid_fluid_3d` スロットを追加した(28箇所の
  構築サイトを更新)。**現時点でこれを使う結合はまだ無い**——`GridFluidRigid` 等は
  2D専用で、3D版の結合は設計の結合行列にも無い。ドメインを一級市民として置くための
  枠であることを `domain_states.rs` に正直に記録した。
- シーンJSONへ `grid_fluid_3d` セクションを追加(`boundary` / `solids`(Box・Sphere)/
  `vorticity_confinement_epsilon` / `smoke_blocks`)。新シーン
  `scenes/d14c-smoke-3d.json`(流路+球の固体境界+煙+渦度強化)を追加した。
- エディタの Hierarchy へ `grid_fluid_3d_summary()` の概要行を出した。
  **縮約**: 3Dの速度場・煙場をブラウザで可視化する経路(ボリュームレンダリング等)は
  無いので、寸法と煙の総量だけを出す。これが無いと**シーンに3D格子流体があっても
  エディタ上で存在を確認する手段が全く無い**ので、そこだけは埋めた。

**解像度の制約を明示した**: 3Dは前処理なしPCGで 64³ なら1ステップ約800ms
(群10のMGPCGで約207msになったが、テストに載せるには依然として重い)。
ギャラリーの全シーンは検証テストで60ステップ回されるため、シーンJSONの
`GridFluid3DScenarioJson` のdocに「小さめ(24×16×16程度まで)」と明記し、
`d14c` も 24×12×12 に抑えた(マニフェスト検証テストの実測 30.8秒)。

**これで群9の7項目すべてが完了した**。7項目のうち **2件は残置理由が事実誤認**
(`PhaseChangeMorph` の実体変換・フルCCD)、**1件は判断としては正しかったが独立に
実装できた**(マニフォールド持続化)、**4件は単に着手していなかった**
(SAH分割・渦度強化・任意形状の固体境界・3D化)。
...
- **残置1: `PhaseChangeMorph`の実体変換**(融けた剛体を消して流体粒子を生成する)。
  ...
  **(群9での後日訂正)この理由は事実誤認だった**——`DomainStates`は最初から`sph`を
  公開しており`SphFluid::add_particle`も存在していた。設計変更は不要で、群9-6で
  そのまま実装した(上記「増分9-6」参照)。
