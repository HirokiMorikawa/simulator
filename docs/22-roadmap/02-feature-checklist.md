...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  に集中定数抗力モデル(`aero`、Schiller-Naumann補正付き球抗力)を実装し `sim-mechanics` の
  `apply_forces` に配線、F1–F3 を Green 化)
- **作業中**: 力学(`sim-mechanics`)P1 の残り — 浮力(自由表面が必要、F4–F6)・最小CCD
  (下記 §2/§3 の P1 行)
...
- [x] 重力(実装済み)。抗力(球、Schiller-Naumann補正付き、`sim-fluid::aero`+
      `MechanicsSolver::apply_forces`)を実装、F1–F3 Green化。浮力(F4–F6、自由表面が必要)は未実装
...
- [x] エネルギー台帳(残差トレンド監視)— `crates/sim-core/src/ledger.rs::EnergyLedger`
      ...
      で検証
- [ ] 担当テスト Green: M1–M9, M12, M15, F1–F6, T1, T2(M1・M5–M9・F1–F3・T1・T2 Green。
      M12=Box-Box待ち(P2)、M15=最小CCD待ち、F4–F6=浮力(自由表面)実装後)
...
- [x] F3 — `crates/sim-mechanics/tests/p1_analytic.rs::f3_stokes_settling_matches_analytic_formula`
      (浮力未実装のため媒質密度を無視できるほど小さく取り、Δρ≈ρ_particle として隔離検証。
      浮力実装後にF4–F6と合わせて厳密化を検討)
- [ ] F4
- [ ] F5
- [ ] F6
- [ ] F7
