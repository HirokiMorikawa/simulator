...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  Phase 0 の `FallingBody` から `MechanicsSolver` 経由に移行)
- **作業中**: 力学(`sim-mechanics`)P1 の残り — 重力以外の力(抗力/浮力、流体結合待ち)・
  熱ノード基礎・エネルギー台帳・最小CCD(下記 §2/§3 の P1 行)
...
- [ ] 熱ノード(基礎)
- [ ] エネルギー台帳(残差トレンド監視)
- [ ] 担当テスト Green: M1–M9, M12, M15, F1–F6, T1, T2(M1・M5–M9 Green。M12=Box-Box待ち(P2)、
      M15=最小CCD待ち、F1–F6/T1/T2=抗力・浮力・熱ノード実装後)
