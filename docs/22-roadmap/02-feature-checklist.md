...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  続けてヘッドレスデモランナー(設計§8、docs/21-verification/03-demo-scenarios.md)に
  着手。「デモの合格 = ヘッドレステストGreen + 目視チェック」(同docs §7冒頭)だが、
  「フロントエンドの視覚UIが無くてもヘッドレスで合格判定できる」(設計が明記)ため、
  ワークストリームD着手前の前半分だけ先に進める。`crates/sim-world/src/demos.rs`を
  新設し、Phase 1スモークのうちD1(落下時計)・D2(弾道)の2本を実装した。両方とも
  既存の解析解テスト(M1・M2)とほぼ1対1対応するため新規物理実装は不要だったが、
  `Scenario`/`from_scenario`(シーンJSON)が`linear_velocity`(初速)・`atmosphere`
  (抗力)フィールドを持たない縮約スキーマのため、JSON経由ではなく`World`公開API
  (`create_body`・`mechanics_mut()`)を直接使う構成にした。D1は落下時計(高さ20m、
  真空側はM1の解析解とrel<1%で一致、発泡相当の低密度球+標準大気で抗力ON/OFFの
  到達時刻差を確認)、D2は45°弾道(真空側はM2の$v_0^2/g$とrel<2%で一致、抗力ありは
  真空側より到達距離が短くなることを確認)、両方とも初回実装で一発Green化した。
  チェックリストの該当項目は目視チェックが未実施(ワークストリームD未着手)のため
  チェックを入れず、ヘッドレス部分がGreenであることのみ注記した。
  続けてD3(バウンド比べ)・D8(散乱の再現)を追加実装した。D3は異なる素材の床・球を
  混在させると接触ソルバの反発係数合成則まで検証対象に含まれデモの主眼(素材ごとの
  反発係数の違い)から外れるため、床・球を同一素材にした4回の独立試行(ゴム・木材・
  鋼・氷)として実装 — 氷(e=0.1)は跳ね返り高さが小さく(約2cm)跳ね返り検出の
  ヒステリシス(1e-4m)が無視できずrel_err約12%に達することを実装検証中に発見し、
  素材ごとに現実的な許容誤差(氷のみ<15%、他<5%)を採用した。D8は50球を(`World`
  自身の物理乱数用`rng`とは独立な)シーン構築用`SimRng`で散乱させ、同シードで2回
  シーン構築+300step実行した`state_hash()`が一致することを確認、初回実装で一発
  Green化した。D10(摩擦の熱)は`integration_scenarios.rs`の既存テスト
  (`brake_heat_scenario_keeps_world_energy_ledger_residual_small`)が同じ内容
  (鋼のブレーキ板+鋼の箱、運動エネルギー→熱の変換対応表)を既に検証済みと判断し、
  重複実装せずヘッドレス部分をカバー済みと見なした。
  続けてD5(斜面)・D9(冷めるコーヒー)を追加実装した。D5は`sim-mechanics`のM7/M8
  解析解テストと同じ傾斜面構成(箱のローカル+y面が斜面法線に一致する回転)を`World`
  経由で再現し、静止摩擦角未満(10°)では静止し続けること(M7)と、静止摩擦角を
  超える(45°)と解析解$g(\sin\theta-\mu_k\cos\theta)$どおりに滑り出すこと(M8)の
  両方を確認、初回実装で一発Green化した。D9は単一の熱ノード(対流のみ、放射なし)を
  `enable_thermal`経由で`World`に接続し、ニュートン冷却の指数減衰(`sim-thermal`の
  T1解析解テストと同じ式・パラメータ)にrel<1%で一致することを確認、初回実装で
  一発Green化した。
  続けてD4(積み木)・D6(浮き沈み)・D7(風と終端速度)を追加実装し、Phase 1スモーク
  10本のヘッドレス部分を完了した。D4は`sim-mechanics`のM12解析解テストと同じ
  4段の箱スタック構成を`World`経由で再現し10秒静止することを確認(反復回数
  スライダーで崩れる観察は`JOINT_VELOCITY_ITERATIONS`が公開API化されていないため
  対象外)。D6はF4(密度比0.6の喫水深さ)・F5(密度比0.5の振動周期)の両方を
  `StaticWaterRegion`経由で確認 — 実装中、周期判定のゼロ交差の向き(上昇/下降)を
  取り違え半周期を1周期と誤検出するバグ(measured≈0.5×analytic)を発見し、
  `sim-mechanics`のF5テストと同じ「下降方向のゼロ交差」判定に修正して解決した。
  D7はF1(高Re、鋼球+$C_d=0.47$の二次抗力)・F3(低Re、ストークス沈降)の2レジームを
  確認(F2は雨粒の実測値というF1と同じ物理の別パラメータのため対象外)。
  D4・D6・D7とも(バグ修正後は)Green化した。Phase 1スモーク10本全てのヘッドレス
  部分がGreen(目視チェックはワークストリームD未着手のため保留)。
  続けてPhase 2〜3の最初としてD11(振り子と時計)を実装した。M3(小振幅周期)を
  `sim-mechanics`のM3テストと同じ構成(`DistanceJoint`によるワールド固定点への
  一定長ピン拘束)で`World`経由で確認しつつ、二重振り子(`DistanceJoint`を2本連鎖、
  質点2を質点1に接続、大振幅でカオス的軌道)を同一初期条件で2回実行し
  ...
  続けて天体ドメイン(`sim_astro::NBodySystem`)が既に`World`の常時合成ドメインとして
  接続済み(`enable_astro`、`step()`が自動sub-stepする)ことに気づき、Phase 4
  (D19–D33)より先にPα(天体ウェーブ)のD34(太陽系儀)を実装した。8惑星ではなく
  1惑星(円軌道)への縮約(テスト実行時間を抑えつつ同じ核心の物理を検証)で、
  `sim-astro`のA1(ケプラー第3法則、1周期後に円軌道半径を保つことで確認)・
  A2(20周回後もエネルギー・角運動量がほぼ保存)解析解テストと同じ物理を`World`
  経由で再現した。「時間加速の切替を跨ぐリプレイ一致」はレジーム切替
  (`docs/20-integration/06-regime-switching.md`)機構が`World`に未接続のため対象外。
  初回実装で一発Green化した。
  続けてD35(軌道投入)を実装した。`sim-astro`のA3(円軌道速度、vis-viva公式の特殊形)
  テストと同じ2体構成に、円軌道速度の0.9倍の初速(楕円軌道、出発点が遠地点になる)を
  与え、vis-viva公式($1/a=2/r_0-v_0^2/GM$)から導いた長半径$a$によるケプラー第3法則
  の周期$T=2\pi\sqrt{a^3/GM}$分だけ`World`を進めると、衛星が出発点(位置・速度とも)
  付近へ戻ることを確認した — 「周期がケプラー則どおり」という合格基準を、円軌道
  ではなく楕円軌道で(D34のA1確認とは異なる軌道形状で)検証する構成にした。初回
  ...
  続けて、この土台の上で最も前提工事が軽い`BoussinesqBuoyancy`(設計§3「P3: 温度場 →
  流体運動量」)に着手した。設計は`GridFluid2D`本体のセルごとの温度場からの空間的に
  変化する浮力を想定するが、`GridFluid2D`(このプロジェクトの縮約実装)は温度場自体を
  持たない(`grid_fluid`モジュールdoc参照、周期境界の速度場のみ)。そのため、単一の
  `ThermalNode`(シーン全体を代表する「熱源」温度、`PistonGas`の`GasCompartment`
  単一区画と同じ縮約の精神)と周囲温度の差から、空間一様な浮力加速度
  ($f_y=\beta(T-T_{amb})g$、設計docs/11-fluid/02-eulerian-grid.md §4.2の
  $f_y=-\beta(T-T_{amb})g$を、`MechanicsSolver::gravity`の符号規約(大きさで表し
  y方向に$-g$として作用)に合わせて整理した式)を流体の速度場全体(`v`のみ、`u`は不変)に
  一括で加える構成にした。独自の重力パラメータを持たせず`DomainStates::mechanics.gravity`
  (`World`全体で常時有効なドメイン)をそのまま読むことで、`World`の重力設定との食い違いを
  避けた。セルごとの温度差によるプルーム形成は再現できないが、解析的に検証可能な一様加速度
  ...
  Green化した。
  続けてPhase 4のD19(電気工作台、「電池・抵抗・LED・コンデンサ・スイッチの自由配線」
  「合格基準: E3/E4/E5、ジュール熱→温度」)に着手した。`circuit`ドメインは既に
  `World`の常時合成ドメインで、`sim_em::Circuit`が分圧に必要な抵抗、E3放電回路に
  必要なコンデンサ、LEDに相当するダイオード、スイッチを全て素子として持つため、
  新規物理実装なしで「自由配線」(単一`Circuit`に分圧回路+コンデンサ放電回路+
  スイッチ付きLED回路を同時配線)を構築できた。E5(理想電圧源からの純抵抗分圧、
  機械精度で検証)・E3(放電形、$V(t)=V_0e^{-t/R_3C}$、rel<1%)・
  `Command::SetSwitch`で実行中にLED分岐を開閉できること(閉時はダイオードが順方向
  導通しnode電圧>0.1V、開時は閉時の半分未満に低下 — スイッチの開放抵抗は有限
  (理想スイッチの近似)なのでダイオードの指数特性により微小な漏れ電流でも数百mV
  残りうることを実装検証中に発見し、「完全に0V」ではなく「閉時より十分低い」という
  相対比較に緩和した)・`JouleHeat`(前増分のCoupling registry経由)による熱ノード
  温度上昇、を単一シーンで確認した。E4(RLC)は`sim-em`側で既にGreenのため重複実装
  しない。初回実装は開放スイッチの閾値設定で1回引っかかった(上記の発見)ものの、
  修正後は一発Green化した。
  続けてD26(帯電風船、「こすって壁につける」「合格基準: 鏡像力の定性 + 逆二乗の測定」)
  に着手した。設計docs/13-electromagnetism/01-electrostatics-magnetostatics.md §2が
  「導体の誘導電荷・誘電体の分極は鏡像力の近似式($F=-q^2/(16\pi\varepsilon_0d^2)$、
  平板近傍の点電荷、風船が壁に貼りつくデモ用に限定提供)のみ提供」と明記しているのを
  見つけ、10種目のCoupling`ImageChargeForce`として新規実装した(`LorentzForce`と同じ
  「`ChargedBody`という正式な結合型がまだ無いため剛体indexと電荷量をCoupling自身の
  フィールドとして持つ」縮約パターン、平板(壁)は接地導体という理想化で反作用対象を
  持たない「外部」由来の力として扱う)。電荷の符号によらず$q^2>0$なので常に平板側への
  引力になることを含め単体テスト2本で解析式と一致することを確認し、`World`経由の
  D26デモでは、軽量な発泡材の球(実際のゴム風船オーダーの質量に近づけた)が壁に実際に
  引き寄せられて到達すること(定性)と、初期距離2倍で初期加速度が1/4になる逆二乗則を
  確認した。実装検証中に、密度の高い材料(木材)・小さい電荷量の組み合わせでは鏡像力が
  弱すぎて現実的なステップ数内で壁に到達しないことを発見し、軽量な発泡材+風船の摩擦
  帯電オーダーに近い電荷量に調整して解決した。修正後は一発Green化した。
  続けてD21(磁石遊び、「磁石の吸引反発・方位磁針・銅管落下」「合格基準: E1系、
  渦電流の終端速度」)に着手した。「磁石の吸引反発・方位磁針」は既存実装
  (`sim_em::magnetism`の磁気双極子)の別側面のため対象外とし、新規物理を要する
  「銅管落下」(渦電流ブレーキによる終端速度)に絞った。既存の`InductionCoupling`
  (E7の解析解で検証済み)はレール方向がワールドX軸に固定されていたが、レンツ則の
  制動ループ(v→EMF→I→制動力→v)は軸の向きによらず自己無撞着に安定する(線形な
  ループのため初期値・軸の向きに依らず速度をゼロへ戻す性質を持つ)ことに気づき、
  `MotorCoupling`と同じ`axis: Vec3`パラメータへ一般化した(既存のE7テストは
  axis=(1,0,0)を明示的に渡すよう更新、数値結果は変化なし)。この一般化により、
  重力下(axis=鉛直上向き)で導体棒が落下しつつ渦電流で制動され、重力と制動力が
  釣り合う終端速度に収束するという新しい物理レジームを検証できるようになった。
  自由減衰(E7)の時定数$\tau=mR/(B\ell)^2$から導かれる実効粘性減衰係数
...
- **作業中**: ワークストリームB(Phase C)の残りは(a)design上のpre/post 2相分離・
  ...
  (Phase D: パストレースレンダラ、現状`sim-render`は4行の空crate)に着手した。
  設計docs/17-rendering/02-path-tracing.md §8の実装順序「BVH + 拡散/鏡面BSDF + NEE
  (基本パストレ) → 分光・屈折・コースティクス → 参加媒質 → 被写界深度・
  モーションブラー」のうち、最初の一歩として`Ray`・`Sphere`(レイ-球交差)・
  `Lambertian`(拡散BSDF、コサイン重み付き半球サンプリング)・再帰的な`Scene::trace`
  を実装し、R1(白色炉テスト)をGreen化した。BVHは複数物体シーンが実際に必要になる
  まで見送った(解析球1個のみのシーンでは線形探索と同義、`sim-fluid`が固体境界
  セルを「必要になってから」導入したのと同じ判断)。NEE(光源の明示サンプル)も、
  R1の検証対象(環境光が方向に依らず一様)には不要なため後続増分に残した。
  実装検証中の発見: Lambertian BSDFをコサイン重み付き半球サンプリング(pdf=cosθ/π)
  と対にすると`bsdf*cosθ/pdf=albedo`が方向によらず恒等的に成り立つ(重要度
  サンプリングの完全な相殺)ことと、孤立した凸形状(球)は自身を自己遮蔽しない
  ことから、albedo=1のとき統計的収束を待たずに1バウンスで解析値と厳密に一致する
  (rel<1e-9)ことが判明した——設計の解析解テスト表がR1を「分級」(分単位の実行時間)
  と見積もっているのは、この特殊構造を活かさないナイーブなモンテカルロ実装を
  ...
  続けて`Scene`を複数物体シーンへ一般化した。`sphere`/`material`固定フィールドを
  `objects: Vec<SceneObject>`(球+BSDFの組)へ置き換え、`closest_hit`(全物体を
  線形探索し`t`最小の交差を返す)を実装した。BVH(加速構造)自体は、線形探索が
  性能上問題になる規模のシーンが実際に必要になるまで見送った(正しさは線形探索
  でも損なわれないため、`sim-fluid::grid_fluid`の固体境界セルと同じ「必要になって
  から」判断)。実装検証中の発見: 当初「手前の黒球が奥の白色炉球を遮蔽する」という
  `trace`経由の統合テストを書いたが、複数物体シーンでは拡散(Lambertian)球からの
  再帰的バウンスが(単独球のときと異なり)他の物体に当たる可能性があるため、
  R1と同じ「1バウンスで必ず環境へ抜ける」という零分散の前提が崩れ、`max_depth`
  超過で不安定な結果になることを発見した。物体選択ロジック(`closest_hit`)自体の
  正しさ(登録順に依らない`t`最小選択・欠側`None`)とBSDFの再帰サンプリングの
  正しさ(既存のR1/R2/誘電体白色炉テストが個別に検証済み)を混ぜて検証しようと
  したのが原因と判明し、`closest_hit`を`pub(crate)`にして直接(`trace`を経由せず)
  幾何選択だけを決定論的に検証するテストに切り替えて解決した。
  続けてNEE(Next Event Estimation、設計§4「直接光: 光源を明示サンプル」)を実装
  した。`PointLight`(位置+放射強度の逆二乗則点光源)を追加し、拡散(Lambertian)
  面でのみ`direct_lighting`(シャドウレイで遮蔽を確認した上で$f_r\cdot(I/d^2)\cdot
  \cos\theta$を合算)を適用する(鏡面/誘電体は反射/屈折方向がデルタ関数のため
  光源の直接サンプルと意味を成さない、標準的な扱い)。光源は幾何を持たない抽象
  光源として`Scene::objects`には含めない設計にしたため、BSDFサンプリングで到達
  したレイが光源自体に「衝突」してNEEの寄与と二重計上する心配が無い(設計§4の
  疑似コードが`sample_lights`と`environment`/BSDF再帰を単純に加算できるのは
  光源が可視物体でない場合に限られる——可視な面光源(エリアライト)には多重重点
  サンプリング(MIS)が必要になるため後続増分)。
  テストは`closest_hit`と同じ理由(複数物体シーンでは間接項が他の物体に当たり
  得るため`trace`経由だと直接照明単体の値と混ざる)で`direct_lighting`を
  `pub(crate)`にして直接検証した: 遮蔽物が無い場合の逆二乗則点光源の解析式との
  厳密一致(rel<1e-9)・遮蔽物がある場合の寄与ゼロ・光源が面の裏側にある場合の
  ...
  続けて設計§7「収束: サンプル数Nでノイズがo(1/√N)減少。決定論(同一シード
  同一画像)」に取り組み、R7を完了させた。この検証だけは他のR番号と異なり
  モンテカルロ推定量そのものの収束率が検証対象であるため、これまでのような
  解析恒等式による分散ゼロ化は使えない(むしろ意図的に分散を持たせる必要が
  ある)。既存の白色炉系テスト群がわざと分散ゼロになるよう設計されていた
  (bsdf*cosθ/pdf=albedoの恒等式)ことを逆手に取り、主球の間接反射方向の先に
  部分的な遮蔽球を置いて「遮蔽球に当たり`max_depth`打ち切りで0を返す/外れて
  環境放射輝度に届く」の二値混合になる検証専用シーンを構築した(この際、
  遮蔽球をカメラと主球の間の光路上に置くと主レイ自体がまず遮蔽球に当たって
  しまうという配置ミスを実装検証中に発見し、遮蔽球をカメラの背後
  (主球から見て間接反射方向のさらに先、原点から見て+z側)に置くよう修正した)。
  この二値混合シーンから2万個の独立サンプル(サブストリームごとに1個)を1回
  引き、バッチサイズ100(200バッチ)とバッチサイズ400(50バッチ)それぞれの
  バッチ平均の分散を比較したところ、実測比は4.16(理論値4に近い、固定シードの
  ため決定論的に同一の値)となり、O(1/N)の分散減衰(=O(1/√N)のノイズ減衰)を
...
- [ ] D1 落下時計(ヘッドレステストGreen、`crates/sim-world/src/demos.rs`。
      ...
      両方(§7冒頭)、この項目のチェックは両方揃うまで見送る)
- [ ] D2 弾道(ヘッドレステストGreen、同上。目視チェック保留)
