...
- **フェーズ**: 実装(Phase 0 完了。math ウェーブは状態非依存の全項目が Green。Phase A 着手済み:
  ...
  P3 力学にジョイント(`joint::DistanceJoint`、1行拘束のみ)を実装し、単振り子として
  M3・M4 を Green 化(大振幅の理論周期は完全楕円積分の自前AGM実装で算出)。
- **作業中**: 力学(`sim-mechanics`)P1 最後の残り — 最小CCD(M15、意図的に後回し)。
  次点候補: BVH(P2 残り)、M10(こまの歳差。固定ピボット回転は DistanceJoint で表現できる
  可能性があり着手可能になった)、磁気双極子・回路MNA・フル RayTracer(sim-em 残り)、
  他ドメイン(quantum)の Phase A スケルトン
...
- [x] ジョイント・拘束(ヤコビアン)— `crates/sim-mechanics/src/joint.rs::DistanceJoint`。
      設計 §4.4 表の Distance(1行拘束、$|\mathbf{p}_B-\mathbf{p}_A|=L$)のみ実装、
      `body_b=None` でワールド固定点への接続(振り子の支点等)も表せる。Ball/Hinge
      (limit・motor)/Slider/Fixed/Wheel・ソフト拘束・真の3×3ブロックソルバ(コレスキー)は
      未実装 — Baumgarte速度バイアス(β=0.2、設計§9)は使うが接触ソルバのような
      split impulse化はしていない
- [ ] XPBD(布・ロープ)
- [ ] 格子流体(MAC・semi-Lagrangian・投影法)
- [ ] 熱伝導網・相変化(エンタルピー法)・気体区画
- [ ] 並列リダクション(同一スレッド数で決定的 — C-1 案 1)
- [ ] 担当テスト Green: M3, M4, M13, M14, F7–F9, F11, T3, T5, T7(M3・M4 Green)
...
- [ ] 担当テスト Green: E1–E7, E9–E12, F10, S4–S6, T8(E1・E2・E9–E12・S4・S5・S6 Green)
...
- [ ] レジーム切替(時間加速)
- [ ] 1PN 補正(オプトイン)
- [ ] 担当テスト Green: A1–A10(A1・A2縮約版・A3・A7 Green。A4–A6・A8–A10=軌道力学/1PN未実装)
