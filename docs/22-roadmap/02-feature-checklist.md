...
- フロントエンドは6パネルドッキングレイアウト(Scene View/Hierarchy/Inspector/
  ...
  D34/D35の16シーンをJSONアセット化、`loadScene`で切替可能)まで実装済み。
  D9/D34/D35は力学剛体を1つも持たない初のギャラリーシーン(熱/天体ドメイン
  のみ)であり、これに伴い`sim-wasm`/`main.ts`双方の「ボディ0個」対応
  (増分B3、詳細は§7 D9の行)を実装済み。

**カウント(本増分後の実測値)**: チェックボックス総数 **262** / 済 **221** / 未 **41**
(完了率 84.4%)。節別内訳:
...
| §7 デモ合格管理表 | — | — | 26 | うち22件が「ヘッドレスGreen、目視チェック保留」で同一理由滞留(増分B2でD1/D2/D3/D7/D21/D26、増分B3でD9/D34/D35、**増分C6でD40/D41/D42/D43**を目視チェック完了)。D24は新規物理待ちでスコープ外明記 |
...
**次の一手**: **増分群E(フロントエンド、E1–E4)・F(設計項目、F1)まで完了**。
残りは下表のとおり。以前の記録: **増分群D(CIゲート)完了 — 横断DoD①〜④が全て達成**
