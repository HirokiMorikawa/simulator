...
- **ワークストリームD(フロントエンド)**: Phase 0スタブから、6パネルドッキングレイアウト
  ...
  (単純スタック、Redoは対象外)も実装済み。残りは回転/スケールのGizmo・
  Redo・残りのオーバーレイ種別・回路エディタ等。
...
- [ ] Undo / Redo(Edit モードのみ)
      (Undoのみ実装済み——Gizmoドラッグ開始のたびに直前の位置を単純なスタック
      (上限20件)へ積み、ToolbarのUndoボタン(Editモードかつスタックが空でない
      場合のみ有効)クリックで1件ずつ`set_body_position_at`により復元する。
      設計が定める「編集操作をシーンJSONの差分として保持」ではなく縮約実装
      (位置のみ、Redoは対象外)。Playwrightで、ドラッグ前は無効・ドラッグ後は
      有効になり、クリックすると実際にドラッグ前の位置へ正確に戻ること・
      スタックを使い切ると再び無効になること・Playモードへ切替えるとスタックに
      履歴が残っていても無効になることを確認した)
