...
| 5 | 結合の縮約解消(apply_pre移行で1step遅れ解消・剛体↔熱ノード対応・自然対流・揚力) | **完了**(PhaseChangeMorphの実体変換は残置、下記) |
| 6 | レンダラの縮約解消(NEE+MIS・ロシアンルーレット・三角形メッシュ・PNG deflate・コースティクス合成/分光) | **完了**(SAH分割は残置、下記) |
| 7 | 場のソルバの縮約解消(FDTD PML・熱伝導 ρc_p/可変伝導率/3D・回路フォールバック連鎖/モータ・GridFluid開境界) | **完了** |
