...
- ホットスポット: 陰的熱伝導の線形ソルバ(PCG)、格子温度場の拡散。
- 目標アルゴリズムとオーダー: 陰的 Euler + PCG(マルチグリッド前処理で ~$O(n)$)。ノード網は疎で安価。
- SoA レイアウト: 温度場は Grid3、リンクは配列。格子はタイリング。
- 並列化単位: 格子スライス/タイル、ノード網の行更新。
- SIMD 対象カーネル: 7 点ステンシル(拡散)、放射線形化。
- GPU 適性: 中〜高(大規模温度場は GPU 向き)。
- ベンチ: 1D/3D 棒の過渡伝導(T3)・冷却(T1)。
