...
- ホットスポット: 物性照会(読み取りのみ、ホットループから頻繁にアクセス)。
- 目標アルゴリズムとオーダー: MaterialId インデックスで $O(1)$ 参照。ペア表は BTreeMap で決定的。
- SoA レイアウト: DB は不変・連続配置。頻繁参照列(密度・摩擦)をキャッシュ効率よく。
- 並列化単位: 不変 DB は読み取り並列で競合なし。
- SIMD 対象カーネル: 該当なし(スカラー照会)。
- GPU 適性: DB を GPU 定数バッファに複製(パストレ・格子カーネル用)。
- ベンチ: 派生量検算(音速・熱拡散率)は §5 のテスト。
