    ...
    experiments: [
      ...
      },
      {
        ...
        watch: [
          ...
          "はじめの振れはばはわざと小さくしてあります(3 度——ひも 1 m なら左右 5cm ほど)。小さく揺らすほど、1 往復の時間が「ひもの長さだけで決まる」という関係がぴったり成り立つからです。",
          "揺れを目で見たいときは「振れはば」を大きく。ただし大きく振ると、上の関係からは少しずつ外れていきます。",
        ],
        view: "3d",
        pace: 120,
        prepare: (scene) => {
        ...
        knobs: [
          ...
          },
          {
            ...
            value: 3,
            // 既定(3°)は、出荷しているシーンJSONに焼き込まれた 0.05 rad
            // (=2.9°)にいちばん近い目盛り。**JSON そのものは変えない**
            // ——Rust 側の受け入れテストが、このファイルの小振幅で
            // 「周期 = 2π√(長さ/重力)」を 1% 以内で確かめている。つまみは
            // 読み込んだ後の場面に効くだけなので、その検証はそのまま成立する。
            hint: "大きいほど目で見て分かる揺れになります。ただし大きく振ると、1往復の時間が「ひもの長さだけで決まる」関係から外れていきます(60度で約7%長くなります)。",
            apply: (scene, value) => {
