 ...
 * **自分で置く物に選べる材質**の一覧。
 *
 * ここに並べた名前が、ツールバーのスポーン材質・Inspector・「選んだもの」札の
 * 材質欄・Materials タブの表の、**すべての出どころ**になる
 * (`materialNames()`/`materialsRef`)。
 *
 * **4種類しか出していなかった**(鋼・アルミ・木・ゴム)。一方、物性を持って
 * いる Rust 側の材質DB(`MaterialDb::standard()`)には13種類あり、見た目の色
 * (`MATERIAL_COLORS`)も13種類ぶん用意されていた——**持っているのに、選ばせて
 * いなかった**。利用者役(「いくつもの物を組み合わせて、それらしい世界を
 * 組み上げたい」人)の観察: 「材質は4種類のプリセットのみで、それぞれ色は
 * 決まっていた。自由な色や質感を選べる場所は見つけられなかった」。
 *
 * 色そのものを自由に塗らせるのではなく、**材質を選べば色も物性も一緒に付いて
 * くる**という今の仕組みのまま、選べる幅を実際にあるところまで広げる
 * ——ガラスは青みがかって軽く割れそうに、銅は赤く重く、発泡スチロールは白く
 * ほとんど重さが無い。見た目と手触りが食い違わない。
 *
 * **「水」と「空気」だけ外す**。どちらも材質DBにはあるが、これは*流体*で、
 * 箱や球として置くための道具は別にある(「＋ 追加」の「+ 流体」)。固体の
 * 塊として「水の箱」を置けてしまうと、その場で嘘になる。
 */
